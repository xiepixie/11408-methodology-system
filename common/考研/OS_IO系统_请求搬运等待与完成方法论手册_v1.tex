...
\large 从系统调用、Polling、Interrupt、DMA 到 Buffer、Page Cache、Spooling 与磁盘调度的统一方法论}
...
\begin{corebox}[本册总纲：I/O 的本质不是“设备大全”，而是异步世界如何接入同步执行核心]
\begin{center}
...
数据到底从哪里移动到哪里？谁执行搬运？
\[
\text{Device}\xrightarrow{\text{DMA}}\text{RAM/Page Cache}
...
\begin{eng}[工程校准：MMIO 比“某个固定端口”更通用]
...
高速网卡、NVMe 等设备可能每秒完成海量小请求。若每个完成都单独中断，CPU 会被中断风暴淹没。因此现实系统常批量完成、合并中断或使用 polling 与 interrupt 的混合模式。它们改变的是工程 fast path，不改变 408 的核心模型：\kw{完成事件必须被系统发现，并映射成软件状态更新}。
\end{eng}
...
\begin{eng}[工程校准：DMA address 不一定等于 CPU physical address]
...
OS 系统重点 & 流控、字符缓冲、终端行规程、事件到达 & 块队列、磁盘调度、Page Cache 缓存、块映射 \\
\bottomrule
\end{tabularx}
\endgroup
实际设备/驱动接口比这更丰富，但这一区分有助于理解为什么磁盘能形成 Page Cache 与块层，而键盘不适合用“随机访问块”理解。
...
\subsection{错误与完成码：I/O slow path 不只有“成功”}
...
Cache & 慢对象可能再次被访问 & 保存副本以复用，命中时绕过慢层 & Page Cache、CPU Cache、TLB \\
...
\section{第 8 章：Page Cache——文件 I/O 的内存 fast path}
...
\paragraph{Hit}
\[
\text{read}\arr\text{Page Cache Hit}\arr\text{copy to user}\arr\text{return}
\]
无底层存储读取。

\paragraph{Miss}
\[
\text{Page Cache Miss}\arr\text{allocate/cache page}\arr\text{submit storage I/O}
\]
...
\[
\text{User Buffer}\arr\text{Page Cache}\arr\text{Dirty}
\]
然后系统在稍后：
\[
...
这是文件系统、Page Cache 和存储设备层次最重要的边界之一。应用若需要明确的持久性语义，需要使用相应 durability 接口；文件系统/块层还可能需要 flush/FUA 等机制把设备易失缓存推进到非易失介质。
\end{warn}
...
\subsection{回收 Page Cache：问“丢掉以后怎样恢复”}
\begin{itemize}
...
Page Cache 与 VM 在 \code{mmap} 处直接汇合：文件页可以通过页表映射进进程虚拟地址空间。普通 \code{read()} 常见路径是 Page Cache $\to$ 用户匿名 buffer 的显式复制；file-backed \code{mmap()} 则可以让文件页直接参与虚拟地址映射。二者最终都依赖页表、缺页和物理页生命周期。
\end{bridge}
...
\begin{eng}[现代 Linux 校准]
现代 Linux 块层支持多种 I/O scheduler，如 mq-deadline、BFQ、Kyber 或 none 等；选择取决于设备和目标。工程层变化很快，但不改变教材母问题：\kw{多个 I/O 请求在有限服务能力下如何排序，并权衡吞吞、延迟和公平性}。
\end{eng}

\section{第 12 章：一次阻塞 \code{read()} 的一生——整册超级母题}
...
read(fd, buf, 4096);
\end{lstlisting}
文件描述符合法，但所需文件页\kw{不在 Page Cache}，底层设备采用 DMA，调用采用经典阻塞式语义。
...
\node[m2, right=of s1] (s2) {2. Page Cache Miss\\[2pt]\small 构造块请求与 DMA};
\node[m3, right=of s2] (s3) {3. 进程 A 阻塞\\[2pt]\small 切换到进程 B};

\node[m4, below=of s3] (s4) {4. 存储设备 DMA\\[2pt]\small Data $\rightarrow$ Page Cache};
...
$t_2$ & Page Cache Miss，提交块请求 & 分配/构造目标缓存页与描述符 & A: 尚未能完成返回 \\
$t_3$ & 驱动启动 DMA，调度器转去 B & 设备开始执行 DMA 数据传输 & A: Blocked, B: Running \\
$t_4$ & CPU 运行进程 B / C 的其他指令 & Device $\xrightarrow{\text{DMA}}$ Page Cache & A: Blocked (在等待队列中) \\
...
$t_7$ & 内核完成路径执行 & Page Cache $\xrightarrow{\text{copy}}$ User Buffer & A: Kernel Running \\
$t_8$ & 系统调用返回用户态 & 用户进程可直接读 buf 数据 & A: User Running \\
\bottomrule
\end{tabularx}
\endgroup

\begin{corebox}[这道母题串起了什么？]
\[
\boxed{
\begin{aligned}
&\text{Syscall}+\text{Process State}+\text{Scheduler}+\text{VFS/File}+\text{Page Cache}\\
...
Cache vs Page Cache & CPU Cache 缓存物理/地址数据访问的硬件层内容；Page Cache 是 OS 管理的文件内容缓存。\\
...
\begin{enumerate}
  ...
  \item \kw{走 fast path 还是 slow path？} Page Cache hit/miss、队列空闲/拥塞？
...
Page Cache 读写逻辑 & File Offset $\to$ Cache Hit/Miss；读路径与 dirty/writeback 写路径分开 \\
...
\text{interrupt} & \text{MMIO/port} & \text{DMA} & \text{page cache} & \text{disk scheduling}\\
...
\begin{itemize}
  ...
  \item Page Cache 与 read/write 的接口理解，用于跨章节综合；
...
\begin{itemize}
  ...
  \item Page Cache hit/miss、readahead、dirty/writeback；
  \item \code{write()} return、writeback、durability 三个时间点；
  \item 设备成本模型改变以后调度策略为何必须变化。
\end{itemize}

\subsection{Engineering Extension：用来校准，不反客为主}
\begin{itemize}
  ...
  \item folio、现代 Page Cache/writeback 实现；
  \item O\_DIRECT、io\_uring、异步完成队列等现代接口。
\end{itemize}

\section{第 16 章：与其他专题的接口}
...
Page Cache、DMA buffer、用户 buffer、\code{mmap} 都会牵涉：
\[
...
\]
两者在 Page Cache、块层和 \code{read/write/mmap} 路径处汇合。

\section{结语：把 I/O 看成一套异步状态转换系统}
传统章节容易留下这样的知识碎片：
\begin{quote}
...
\newpage
\section*{参考与工程校准资料}
\addcontentsline{toc}{section}{参考与工程校准资料}
