...
这只是：
\[
\boxed{\text{Mode Switch (模式切换：仅换特权级与内核栈，不换进程，不换页表)}}
\]
...
这才是：
\[
\boxed{\text{Context Switch (进程上下文切换：换通用寄存器现场 + 换页表 CR3)}}
\]
...
\midrule
1. 进入内核态 $\neq$ 切换进程 & \textbf{模式切换 (Mode Switch)} 只是 CPU 执行特权级的改变（用户态 $\leftrightarrow$ 内核态），并换用进程私有的内核栈。\kw{进程的 PID、虚拟地址空间（页表基址 CR3）完全没变}，CPU 依然在为原进程执行内核服务代码。\par\textit{典型反例}：\code{getpid()}、\code{time()} 或读已缓存数据的 \code{read()}，内核在几微秒内执行完毕，直接通过 \code{sysret/iret} 原路返回同一进程。 & 当前进程在内核中因 I/O、锁或信号量进入 \textbf{Blocked}（如读取磁盘块），或时间片耗尽被抢占，内核调度器才会把 CPU 换给另一个进程。\\
\midrule
...
\midrule
4. 阻塞 (Blocked) $\to$ 必然引发调度 & \textbf{阻塞的本质是“即使给 CPU 也无法继续推进”}（在等磁盘读完、等网络包到达或等锁释放）。操作系统决不能允许 CPU 在内核中白白空转死等，内核必须将当前进程置为 Blocked 并挂入等待队列，\kw{立刻主动调用调度器将 CPU 让给其他就绪任务}。 & \kw{只要发生真正的阻塞}，系统就必然触发调度机会；若此时没有其他就绪用户进程，内核会调度执行专用的空闲进程（\code{idle task}）进入低功耗等待。\\
\bottomrule
\end{tabularx}
\endgroup
...
因此，\kw{线程的第一本质不是“被调度”，而是“可独立保存与恢复的执行上下文”}：
\[
\boxed{
...
\subsection{PCB 的组织方式：本质是“怎样按身份/状态快速找到控制记录”}
...
真正的不变量是：\kw{内核必须能按 identity 找到对象，并能按 scheduling/wait state 找到候选集合；状态改变时，控制记录与相应索引/队列必须一致地改变。} 所以“PCB 组织方式”应挂在 Objects/Relations/Queues，而不是另起一套死记表。
...
\]
例如一次真正的阻塞不能只写成 \code{state=Blocked}，它至少意味着：
\[
\text{Running on CPU}
\rightarrow
...
\item \kw{Blocked $\not\to$ Running}：事件完成只消除了等待条件，使其重新具备运行资格；真正获得 CPU 还缺 scheduler/dispatch；
...
\]
比值小表示系统“欠”该主体的 CPU 份额更多，应提高其获得 CPU 的机会。真正值得留下的不是某一串调度序列，而是：\kw{任何公平性命题都必须先说清公平对象和应得份额，否则“公平”没有可验证含义。}
...
切换维度 & 状态变化 & 核心动作与代价\\
\midrule
1. Privilege / Mode Switch & 用户态 $\longleftrightarrow$ 内核态 & 仅改变 CPU 当前特权级与堆栈指针（切入内核栈），\kw{不更换当前执行流，不更换虚拟地址空间}\\
...
\renewcommand{\arraystretch}{1.25}
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\linewidth}{>{\bfseries}p{0.26\linewidth} c c c c}
\toprule
...
\subsection{M:1、1:1、M:N 的本质：两层实体集合之间的映射}
...
\end{itemize}
\kw{本质启示}：\kw{一旦状态被切分在两个管理层次，就必须建立显式的跨层事件同步协议}。这一思想在后来的虚拟机 Hypervisor、容器运行时以及现代 GPU 运行时中被反复复用。
...
\subsection{现代 Runtime 的本质复活：执行上下文表示论 \texorpdfstring{$(Object \neq Representation)$}{(Object != Representation)}}
...
\subsection{线程做题九问控制器 (Thread Control Adapter)}

遇到任何线程共享、ULT/KLT、M:N、阻塞传播、多核并行或线程切换考题，按以下九步严格推演：

\begin{enumerate}
\item \kw{当前对象处于哪一层？}
  区分 Process Resource Context ($R$)、User Execution Context ($U$)、Kernel Schedulable Context ($K$)、Thread Group ($G$) 还是物理 CPU ($C$)？
\item \kw{这条执行流属于哪个资源世界？}
  其虚拟地址空间（页表基址）、代码段、数据段、堆与打开文件表属于哪个进程？
\item \kw{哪些状态是 shared，哪些状态是 per-thread？}
  牢记：代码/堆/全局变量/文件表共享；PC/寄存器/SP/栈帧/TLS/信号屏蔽字独立私有。
\item \kw{题目问“私有/共享”时是在问 Ownership、Addressability 还是 Concurrency Safety？}
  所有权归线程上下文；同一地址空间内其他线程只要拿到指针就能直接访问（无硬件隔离）；但并发修改必须加锁同步。
\item \kw{谁能看见并调度它？}
  用户态运行时维护 User TCB 调度 $U$；内核调度器维护 Kernel TCB 调度 $K$。内核看不见纯 ULT。
\item \kw{U 到 K 的映射模型是什么？}
  M:1、1:1 还是 M:N？两级队列 $Q_U$ 与 $Q_K$ 的容量关系如何？
\item \kw{当前状态属于哪一级？发生的事件真正导致等待了吗？}
  User-Ready 还是 Kernel-Blocked？Syscall 与 Page Fault 不天然等于 Blocked；若阻塞，标出哪个底层 Carrier 失去运行资格。
\item \kw{事件的作用域 (Event Scope) 是单线程还是整个线程组？}
  单线程退出 (\code{pthread\_exit}) vs 进程级退出 (\code{exit})；线程定向信号 (SIGSEGV) vs 进程定向信号 (SIGINT)。
\item \kw{若发生切换，究竟换了什么？还剩多少物理并行算力？}
  Mode Switch、Execution Context Switch、Memory Context Switch 还是 Mapping Change？用 $P_{\text{actual}} \le \min(U_{\text{runn}}, K_{\text{runn}}, CPU_{\text{avail}})$ 精确计算剩余并行度。
\end{enumerate}

\begin{exam}[线程题的最短定性口诀]
\[
\boxed{\text{先分层，再看谁能看见；先找承载，再谈阻塞；先定调度实体，再谈时间片。}}
\]
\end{exam}
