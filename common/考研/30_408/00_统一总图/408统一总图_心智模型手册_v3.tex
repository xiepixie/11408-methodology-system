...
\subsection{Cost 不是单个 \texorpdfstring{$O(\cdot)$}{O(·)} 标签}
...
\section{数据结构最重要的概念边界：不是背“\texorpdfstring{$\neq$}{!=}”，而是学判据}
...
\]
必须维持物理与逻辑不变量 $I(S_{t+1})=\mathrm{true}$ 成立，同时优化 Cost。
...
\subsection{\texorpdfstring{Name $\rightarrow$ Address $\rightarrow$ Forwarding $\rightarrow$ Delivery}{Name -> Address -> Forwarding -> Delivery}}
\[
...
\subsection{\texorpdfstring{Statistical Multiplexing $\rightarrow$ Queue $\rightarrow$ Congestion $\rightarrow$ Feedback}{Statistical Multiplexing -> Queue -> Congestion -> Feedback}}
