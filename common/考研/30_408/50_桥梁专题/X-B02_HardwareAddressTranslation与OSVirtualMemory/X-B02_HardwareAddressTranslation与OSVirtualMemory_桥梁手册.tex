...
\[
\boxed{\text{Mapping Decision}\to\text{PTE/Page-Table State}\to\text{MMU/TLB Consumption}\to\text{Result or Fault}\to\text{OS Repair}\to\text{Retry}}
\]
这条箭头表示一次地址访问的因果与控制权交接顺序。OS 不直接“翻译每个地址”；它把地址空间、驻留、权限和属性编码进硬件可消费的映射状态。硬件先消费这些状态，只有当访问不能按当前状态合法完成时，才以体系结构可观察的 fault 把控制权交回 OS。
\end{corebox}

\begin{methodbox}{定位、Owner 与停止边界}
\begin{itemize}
\item \kw{左侧 Owner：}CO07 地址翻译硬件，负责 TLB、page walk、PTE 可见字段、VA\(\to\)PA 与访问权限检查。
\item \kw{右侧 Owner：}OS03 Virtual Memory，负责地址空间、页表构造、页分配/驻留、缺页处理、COW 与置换策略。
...
\section{三类缺失必须分流}
\begin{center}\small
\begin{tabularx}{\linewidth}{>{\bfseries}p{2.3cm}YYY}
\toprule
事件 & 缺失对象 & 主要处理者与下一步 & 是否必然进入 OS\\
\midrule
TLB miss & 翻译缓存副本 & 硬件或软件 page walk，成功后 refill & 否；可能完全由硬件完成\\
Page/permission fault & 合法映射、驻留或访问权限 & 精确异常交给 OS，修复、拒绝或终止 & 是（对需要软件处理的架构路径）\\
Cache miss & 数据块副本 & Cache fill、替换或写回，再重试 Cache access & 否\\
\bottomrule
\end{tabularx}\end{center}

因此一次访问可以先 TLB miss、page walk 成功，再 Cache miss；也可以在翻译权限失败处停止。\kw{“没有找到”不是同一种事件：先问缺失的是翻译状态、合法性还是数据副本。}

\begin{warnbox}{两个必须阻断的类比}
\begin{itemize}
\item \(\text{TLB miss}\neq\text{Page Fault}\)：前者是翻译缓存的局部副本缺失，后者是当前访问不能按架构权限/映射继续。
\item \(\text{Hardware Cache}\neq\text{OS Page Cache}\)：前者保存访问路径上的数据副本；后者由 OS 管理文件/存储数据与驻留策略。二者都“缓存”，但对象、Owner 和修复事件不同。
\end{itemize}
\end{warnbox}

\section{架构证据：以 RISC-V 为锚点，不冒充所有 ISA 相同}
RISC-V 监督态规范给出了适合本 Bridge 的最小可观察协议：trap 进入 S-mode 时，\code{sepc} 保存被中断或发生异常的指令地址，\code{scause} 保存事件代码；原因表区分 U-mode/S-mode environment call、load/store page fault 等事件。\code{SRET} 根据 \code{SPP} 恢复到陷阱前的特权级；这使“fault → 处理 → 返回原执行位置/分支处置”成为可检查的接口，而不是抽象地说“回到用户态”\footnote{RISC-V ISA Manual, \href{https://github.com/riscv/riscv-isa-manual/blob/main/src/priv/supervisor.adoc}{Supervisor trap and address-translation registers}.}。

这只是一个架构实例。x86、ARM 和 RISC-V 在寄存器名称、页表格式、硬件/软件 page walk 与返回指令上不同；408 层面保留共同接口：\(\text{保存现场}\to\text{识别原因}\to\text{修复或拒绝}\to\text{恢复/终止}\)。遇到具体 ISA 题目，必须回到该 ISA Owner 的入口协议。

\section{调用协议：看到跨科题时怎么起手}
\begin{enumerate}
\item 先写访问请求 \((VA,读/写/取指,特权级)\)，不要只画一个箭头。
\item 标出 OS 预先提供的 mapping：根页表、层级 PTE、有效位、权限与驻留/属性状态。
\item 追踪硬件结果：TLB hit、page walk 成功、permission fault 还是 page fault。
\item 若成功，才把 PA 交给 Cache/内存；若失败，写明 fault 原因、保存现场和 OS 的修复/拒绝分支。
\item 修复后检查旧翻译是否仍可能被消费，并明确 retry 的指令/阶段；不要把“返回”误写成“必然重新调度”。
\end{enumerate}
