...
\begin{methodbox}{Owners 与 Stop Boundary}
\begin{itemize}
  \item \kw{Left Owner：}CO-02 Own ISA contract、编码、EA、访存宽度、Next PC 与异常语义。
  \item \kw{Right Owner：}CO-03 Own 值依赖图、路径、资源调度、控制字、关键路径和提交。
  \item \kw{Bridge Owns：}规范化状态差、接口字段完整性、语义到实现需求的翻译顺序与双向校验。
  \item \kw{Stops：}不重讲 C/ABI，不选择具体 MUX/总线，不讨论多指令 hazard；后两者分别留给 CO-03/04。
\end{itemize}
\end{methodbox}
