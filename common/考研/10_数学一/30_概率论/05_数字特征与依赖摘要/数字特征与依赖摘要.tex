...
\begin{corebox}{母问题：怎样高效压缩概率分布而不遗忘关键结构与信息损失？}
完整概率分布包含了无限维信息。数字特征是用少数几个关键标量（中心位置、波动幅度、线性关联度）对分布进行最佳数值压缩。

主线推演逻辑：
\[
\boxed{
\text{分布 } P_X \xrightarrow{\text{加权平均}} E(X) \xrightarrow{\text{二阶中心矩}} \operatorname{Var}(X) \xrightarrow{\text{交叉二阶矩}} \operatorname{Cov}(X,Y) \xrightarrow{\text{标准化}} \rho_{XY}
}
\]
\end{corebox}
...
\begin{itemize}
  \item \kw{本册拥有（Owns）：}数学期望 $E(X)$、方差 $\operatorname{Var}(X)$、协方差 $\operatorname{Cov}(X,Y)$、相关系数 $\rho_{XY}$、矩（原点矩与中心矩）、条件期望 $E(Y \mid X)$ 与全期望公式。
...
\[
\mu'_r=E(X^r),\qquad m_r=E(|X|^r),\qquad
...
\[
\operatorname{CV}(X)=\frac{\sigma}{|\mu|}\quad(\mu\neq0)
\]
...
由于最大特征值为 $1+|\rho|$，
\[
\max_{a^2+b^2=1}\operatorname{Var}(aX+bY)=1+|\rho|.
\]
...
若二阶矩存在，
\[
E|XY|\leq\sqrt{E(X^2)E(Y^2)},\qquad |\operatorname{Cov}(X,Y)|\leq\sigma_X\sigma_Y.
\]
...
若 $X\ge0$，事件指标满足 $I_{\{X\ge a\}}\le X/a$，取期望即得 Markov。对一般 $X$，把 $|X-\mu|\ge\varepsilon$ 应用于非负量 $(X-\mu)^2$，得到 Chebyshev。单侧 Cantelli 则对任意 $c>0$ 写出
\[
...
\begin{enumerate}
  ...
  \item 检查存在性：$E|X|<\infty$ 才能谈有限期望，$E(X^2)<\infty$ 才能使用方差和相关系数；重尾模型先停在边界检查。
  \item 选择最短表示：函数期望优先 LOTUS，计数优先示性变量，分层信息优先全期望/全方差，线性组合优先协方差展开。
  \item 只在需要时使用独立：期望线性性不需要独立；乘积拆分、卷积、交叉协方差消失才需要相应独立或不相关条件。
  \item 做结果校验：方差非负，$|\rho|\leq1$，条件方差分解两项非负，单位与尺度变换一致，概率界不超过 $1$。
\end{enumerate}
