...
> 类型：Atlas
> 状态：待人工确认；README 是 Canonical Subject Atlas 候选，八个 Topic 仍是 Source 工作稿。考试动作另见[概率统计做题规则](../90_学科做题规则/概率统计.md)。
>
...
这些文件已经承担工作态解释责任，但尚未经过使用者逐册确认，因此不是“已采用”资产。
...
使用者已授权依据本 Atlas 和旧笔记建立八个 Topic 工作稿，因此本轮已执行 Topic 切分；这不等于下面的长期表达已经被采用：
...
人工确认前，Atlas 与 Topic 均保持“工作稿”。旧笔记中的公式表、题型技巧和扩展内容只作为输入材料；未通过边界、反例和调用检查的内容不自动进入 Canonical。
