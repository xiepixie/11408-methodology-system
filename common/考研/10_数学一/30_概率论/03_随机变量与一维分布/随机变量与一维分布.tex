...
$f_X(x)$ 是单位长度上的概率强度，可以大于 $1$；概率是区间面积。CDF 连续只能说明没有点质量，不能单独证明存在通常意义下的密度；混合分布可同时含跳跃和连续部分。
\end{warnbox}
...
\begin{methodbox}{变换后的质量账本：常值纤维、连续纤维、多原像}
对分段函数 $Y=g(X)$，先把原支撑按 $g$ 的行为分成三类：
\begin{itemize}
  \item $g$ 在一段上为常数：该段的全部概率压到一个点，形成原子；
  \item $g$ 在一段上单调且非恒定：该段贡献连续质量，按原像和 Jacobian 计算；
  \item 同一 $y$ 有多个非恒定原像：各分支密度相加，但不能把不同分支的质量重复计入。
\end{itemize}
因此“输入连续”不保证输出没有原子，“变换分段”也不等于每段都产生原子。最后必须分别回检原子质量、连续密度积分和总质量。
\end{methodbox}

\begin{examplebox}{2013 真题：两个原子与一段连续质量必须同时交付}
设 $f_X(x)=x^2/9$（$0<x<3$），并令
\[
Y=\begin{cases}
2,&X\le1,\\
X,&1<X<2,\\
...
\]
中段恒等映射保留连续密度 $f_{Y,c}(y)=y^2/9$（$1<y<2$），其质量为 $7/27$。因此完整 CDF 必须写成
\[
F_Y(y)=\begin{cases}
0,&y<1,\\
\displaystyle\frac{y^3+18}{27},&1\le y<2,\\
1,&y\ge2,
\end{cases}
\]
其中 $F_Y(1)=19/27$ 已包含 $Y=1$ 的原子，而在 $y=2$ 处从 $26/27$ 跳到 $1$，跳跃高度为 $1/27$。更稳妥的写法是同时标注“原子 + 连续密度”，避免把第一段的常值误看成连续 CDF。

若只求 $P(X\le Y)$，不必再构造联合分布：在 $X\le1$ 层有 $Y=2$，在 $1<X<2$ 层有 $Y=X$，而 $X\ge2$ 层有 $Y=1<X$，所以
\[
P(X\le Y)=P(X<2)=\int_0^2\frac{x^2}{9}\,dx=\frac8{27}.
\]
这道题的停止条件是：完整 CDF 问题在“两个跳跃、连续段密度、总质量为 $1$”闭合后停止；第二问只需按变换分支判断事件，不倒回重求完整联合密度。
\end{examplebox}

\begin{examplebox}{1993 真题：对称与零协方差都不能制造独立性}
设
\[
f_X(x)=\frac12e^{-|x|},\qquad x\in\mathbb R,
\]
求 $E(X)$、$D(X)$、$\operatorname{Cov}(X,|X|)$ 并判断 $X$ 与 $|X|$ 是否独立。
对称性给出 $E(X)=0$，而
\[
...
因为 $x|x|e^{-|x|}/2$ 是奇函数，
\[
E[X|X|]=0,\qquad \operatorname{Cov}(X,|X|)=0.
\]
但 $Y=|X|$ 是 $X$ 的非恒定函数，故不能独立。取同一事件
\[
B=\{|X|\le1\}=\{Y\le1\},\qquad P(B)=1-e^{-1}\in(0,1),
\]
...
\[
Y=aX+b\Longrightarrow f_Y(y)=\frac1{|a|}f_X\left(\frac{y-b}{a}\right).
\]
直接写 $f(ax+b)$ 的积分为 $1/|a|$，不自动是 PDF；$f(x)g(x)$ 通常也需除以总积分后才能归一化。

\begin{examplebox}{早期真题：全局单调变换的支持与 Jacobian}
1988 Q十一中 $f_X(x)=1/[\pi(1+x^2)]$、$Y=1-\sqrt[3]{X}$。由于 $x=(1-y)^3$ 在全实轴上单调，只有一个原像，故
\[
f_Y(y)=\frac{3(1-y)^2}{\pi\{1+(1-y)^6\}},\qquad y\in\mathbb R.
\]
1995 Q十一中 $X\sim\operatorname{Exp}(1)$、$Y=e^X$，原像 $x=\ln y$ 把支持搬到 $y\ge1$，从而
\[
f_Y(y)=e^{-\ln y}\frac1y=\frac1{y^2}I_{[1,\infty)}(y).
\]
两题共同说明：先判全局单调性和新支持，再乘 $|dx/dy|$；不能把支持边界留到最后。
\end{examplebox}
...
\item 变换题先求值域和原像；非单调时枚举全部分支；常值纤维时检查点质量。
...
忽略 $F(a^-)$ & $X=a$ 是否可能有原子？ \\
只取一个反函数分支 & $g(x)=y$ 在支持集内有几个原像？ \\
连续输入必有连续输出 & 变换是否含常值区间？ \\
