...
{\LARGE 心智模型手册 v2\par}
\vspace{0.4cm}
{\large 局部模型 · 连续累积 · 局部--整体 · 无限逼近 · 动态恢复\par}
\vspace{0.8cm}
\begin{tikzpicture}[
...
% 第一排：基础微积分与一元工具层 (x=0, 5.8, 11.6)
\node[atlas] (lim) at (0,0) {\textbf{Limit \& Object}\\\scriptsize 极限底座与研究对象};
\node[atlas] (loc) at (5.8,0) {\textbf{Local Model}\\\scriptsize 局部近似：导数/Taylor};
\node[atlas] (acc) at (11.6,0) {\textbf{Accumulation}\\\scriptsize 连续累积：定积分/重积分};

% 第二排：高阶结构与恢复工具层 (y=-2.2)
\node[atlas] (inf) at (11.6,-2.2) {\textbf{Infinite Build}\\\scriptsize 无限构造：级数与 Fourier};
\node[atlas] (lg) at (5.8,-2.2) {\textbf{Local $\leftrightarrow$ Global}\\\scriptsize 局部与整体：FTC/场论定理};
\node[atlas] (dyn) at (0,-2.2) {\textbf{Dynamics}\\\scriptsize 动态恢复：微分方程轨迹};

% 正向流
\draw[flow] (lim.east) -- node[above,font=\tiny\color{deep}]{局部线性化} (loc.west);
\draw[flow] (loc.east) -- node[above,font=\tiny\color{deep}]{连续累积} (acc.west);
\draw[flow] (acc.south) -- node[right,font=\tiny\color{deep}]{无限展开} (inf.north);

% 底行反向化简流
\draw[flow] (inf.west) -- node[above,font=\tiny\color{deep}]{场与边界定理} (lg.east);
\draw[flow] (lg.west) -- node[above,font=\tiny\color{deep}]{方程轨迹恢复} (dyn.east);

% 左侧闭环上升线
\draw[dashedflow] (dyn.north) -- node[left,font=\tiny\color{accent}]{逼近与全局重建} (lim.south);
\end{tikzpicture}
\vfill
...
\begin{methodbox}{任何抽象第一次出现，都必须能落到四件事}
本册采用固定解释契约：
\[
\boxed{\text{Meaning}\to\text{Chapter Mapping}\to\text{Worked Example}\to\text{Problem Action}}.
\]
...
本册不把每一种积分技巧、每一种含参极限套路、每一种微分方程特解模板全部写成题型百科。那些属于后续 Practice Layer。本册只保留足以\key{选择正确对象、正确表示、正确母模型和正确检查项}的理论与典型推演。
\end{warnbox}
...
参数曲线 $\bm r(t)$ & 用一个参数生成空间轨迹 & 切向、弧长、沿曲线累积 & 空间曲线、线积分\\
...
\begin{itemize}
  \item 一元导数 $f'(x)$ 升级为多元微分/梯度 $\nabla f$，因为现在允许的移动方向不再唯一；
  \item 一元二阶导 $f''$ 升级为 Hessian，因为二阶弯曲需要记录不同方向之间的耦合；
  \item 区间积分升级为区域、曲线、曲面积分，因为“微小单元”从 $dx$ 变成 $dA,dV,ds,dS$；
  \item 一元的“导数积分互联”升级为 Green、Gauss、Stokes，把局部微分算子与边界/区域积分连接。
\end{itemize}
所以新章节的第一问永远是：\key{对象变了以后，原来哪个操作需要升维？}
\end{corebox}
...
\begin{longtable}{L{2.8cm}L{5.0cm}L{4.0cm}L{\dimexpr\linewidth-11.97cm\relax}}
\toprule
...
\section{一条贯穿母例：位置 $s(t)$ 如何串起整门课}
\begin{examplebox}{粒子的运动不是为了讲物理，而是为了展示五种数学动作}
设位置为 $s(t)$：
\begin{enumerate}
  \item 研究 $t\to t_0$ 时 $s(t)$ 是否靠近某值，是\key{极限}；
  \item $v(t)=s'(t)$ 把一个很短时间内的位置变化压缩成瞬时变化率，是\key{局部模型}；
...
\section{高数的第二条暗线：Representation Choice}
高数大量“技巧”其实不是新定理，而是\key{改表示}：
\begin{itemize}
...
\section{数列是定义域离散化后的函数}
数列 $a_n$ 可以看作 $\mathbb N\to\mathbb R$ 的函数。它不能直接使用连续变量的全部微分工具，但极限、单调、有界、夹逼、递推等思想与函数高度相通。递推数列题最重要的不是“猜极限”，而是先证明极限存在，再把可能极限代入不动点方程。
...
数列只有 $n\to\infty$ 的离散接近；函数可以从左、从右、从任意路径接近。多元函数还会进一步增加“从任意空间路径接近”的要求，这正是多元极限更严格的原因。
...
\subsection{隐函数求导：曲线已给出，但没有显式解出 $y$}
...
\]
于是分子主导阶是 $x^2/2$。若只写 $e^x\sim1+x$，会把真正决定答案的二阶信息抹掉。
\end{examplebox}
...
\]
它本质上是\key{选择更适合当前结构的坐标}。以后极坐标、柱坐标、球坐标和一般 Jacobian 都是同一思想升维。
...
\end{itemize}
梯度不是“偏导数组成的向量”这么简单，而是\key{一阶局部变化的完整编码}。
...
\]
这正是高数与线代最重要的接口之一：\key{微积分负责局部线性化，行列式负责局部面积/体积缩放。}
...
\section{局部算子与全局量的配对}
\begin{longtable}{L{2.8cm}L{4.2cm}L{4.4cm}L{\dimexpr\linewidth-11.57cm\relax}}
\toprule
...
\draw[arrow] (ftc.east) -- node[above,font=\tiny\color{deep}]{平面维度提升} (green.west);
\draw[arrow] (ftc.south) -- node[left,font=\tiny\color{deep}]{三维体累积 (散度)} (gauss.north);
\draw[arrow] (green.south) -- node[right,font=\tiny\color{deep}]{曲面侧向提升 (旋度)} (stokes.north);
\draw[dasharrow] (gauss.east) -- node[above,font=\tiny\color{accent}]{散度与旋度对立统一} (stokes.west);
\end{tikzpicture}
\end{center}

\section{Green：二维区域内部的旋转/散度信息与边界积分互换}
平面上最常用的环流型 Green：
\[
...
Fourier 级数在连续点趋向函数值，在跳跃点趋向左右极限平均。这提醒我们：\key{一个表示级数的极限对象不一定逐点等于原定义函数的每一个点值。}
...
\section{Taylor、Jacobian、Hessian 是同一个“局部模型”思想逐阶升级}
\begin{itemize}
  \item 一元一阶：$f(x+h)\approx f(x)+f'(x)h$；
  \item 多元一阶：$f(\bm x+\bm h)\approx f(\bm x)+\nabla f^T\bm h$；
  \item 向量值映射的一阶：Jacobian 是局部线性映射；
  \item 多元二阶：Hessian 通过二次型记录局部弯曲。
\end{itemize}
...
\end{itemize}
很多综合题真正考的是：\key{你是否意识到内部公式只处理内部，而边界必须单独维护。}

\section{高数与线性代数的三个真正接口}
\begin{enumerate}
  \item \key{Jacobian determinant：}微积分给局部线性映射，行列式给局部面积/体积缩放；
  \item \key{Hessian quadratic form：}多元二阶局部模型最终由二次型正负性判极值；
...
\begin{corebox}{重写过程中得到的一个重要理解}
高数表面上最杂，是因为它不断改变\key{对象、维度和表示}；但真正反复出现的操作很少：
\[
\boxed{\begin{gathered}\text{Approximate locally}\quad\text{Accumulate globally}\quad\text{Change representation}\\\text{Pass to a limit}\quad\text{Recover from a law}\end{gathered}}.
\]
...
\begin{longtable}{L{2.2cm}@{}>{\centering\arraybackslash}p{0.5cm}@{}L{2.2cm}L{4.2cm}L{4.2cm}L{\dimexpr\linewidth-13.9cm\relax}}
\toprule
\textbf{概念 A} & & \textbf{概念 B} & \textbf{真正区别} & \textbf{题目信号与判据} & \textbf{混淆后果}\\
\midrule
...
反常积分 & $\neq$ & 普通定积分 & 通过极限定义无限区间/奇点 & $\infty$ 或无界点 & 不写极限直接代端点\\
偏导存在 & $\neq$ & 可微 & 坐标轴方向 vs 全方向统一线性主部 & 原点分段、多元定义题 & 只算两个偏导就下结论\\
方向导数存在 & $\neq$ & 可微 & 每方向单独存在仍可能不统一线性 & “任意方向”与可微 & 把方向信息误当全微分\\
梯度 & $\neq$ & 方向导数 & 梯度是编码向量，方向导数是投影标量 & $\nabla f$ vs $D_{\bm u}f$ & 忘单位方向/点积\\
驻点 & $\neq$ & 多元极值 & $\nabla f=0$ 仍只是候选 & Hessian/边界 & 漏鞍点和边界\\
局部极值 & $\neq$ & 全局最值 & 邻域比较 vs 整个定义域比较 & 闭区域、边界 & 只比较内部驻点\\
二重积分 & $\neq$ & 累次积分 & 前者是区域整体对象，后者是计算表示 & 换序/分区 & 上下限不对应区域\\
坐标变换 & $\neq$ & 纯换变量符号 & 微元也按 Jacobian 缩放 & 极/柱/球/一般换元 & 漏 $r$、$\rho^2\sin\varphi$\\
第一类线积分 & $\neq$ & 第二类线积分 & 标量沿弧长 vs 向量切向投影 & $ds$ vs $Pdx+Qdy+Rdz$ & 方向/参数化错误\\
第一类面积分 & $\neq$ & 第二类面积分 & 标量面密度 vs 向量法向通量 & $dS$ vs $\bm F\cdot\bm n dS$ & 忘法向侧别\\
闭曲线 & $\neq$ & 封闭曲面 & 一维边界 vs 二维边界 & Green/Stokes vs Gauss & 定理选择错误\\
Green/Stokes & $\neq$ & Gauss & 维度、边界对象、局部算子不同 & 平面闭边界/封闭面/空间曲线 & 公式互套\\
$\nabla\cdot\bm F$ & $\neq$ & $\nabla\times\bm F$ & 源汇强度 vs 局部旋转 & flux vs circulation & 物理/几何含义颠倒\\
旋度为零 & $\neq$ & 全局保守场 & 还需适当区域拓扑条件 & 有洞区域、路径无关 & 漏区域条件\\
数列收敛 & $\neq$ & 级数收敛 & 单项趋限 vs 部分和趋限 & $a_n$ vs $S_n$ & 把 $a_n\to0$ 当充分条件\\
...
齐次 ODE & $\neq$ & 一阶齐次型 & 一个是线性右端为零，一个是 $y/x$ 结构 & “齐次线性”/“齐次方程” & 方法完全选错\\
...
\begin{enumerate}
  \item \key{Object：}对象是数列、一元函数、多元函数、曲线/曲面、向量场还是未知函数？
  \item \key{Domain：}定义域、积分域、参数区间、收敛区间、区域边界是什么？
  \item \key{Target：}要求极限、局部性质、累计量、存在性、最值、展开还是解轨迹？
  \item \key{Mother Model：}当前主要是极限底座、局部模型、累积、局部--整体、无限构造还是动态恢复？
  \item \key{Representation：}显式/隐式/参数式/极坐标/势函数/级数展开中，哪种表示最自然？
  \item \key{Structure：}对称、周期、单调、齐次、线性、保守、闭合、正项、交错等信号是什么？
  \item \key{Candidate Routes：}至少有哪两条可行路线？哪条减少复杂度？
  \item \key{Execute：}过程中是否维护定义域、阶数、方向、微元、自由常数、端点？
  \item \key{Local Check：}每个关键转化后能否用定义/导回去/边界检验立即检查？
  \item \key{Global Verify：}最终符号、范围、量纲、极端值、几何方向是否合理？
\end{enumerate}
\end{corebox}
...
\midrule
$x\to a,n\to\infty$、收敛 & 极限/无限过程 & 找主导结构或先证明存在\\
切线、近似、误差、极值 & 局部模型 & 找一阶/二阶局部信息\\
面积、质量、功、总量 & 累积 & 写局部贡献与区域\\
闭曲线、封闭面、路径无关 & 局部--整体 & 判断 circulation/flux/boundary\\
无穷和、展开、和函数 & 无限构造 & 先识别级数类型与收敛域\\
...
\section{执行断点：高数最需要维护的五类“中间状态”}
\begin{enumerate}
  \item \key{阶数状态：}Taylor/无穷小计算当前保留到几阶，低阶是否已相消；
  \item \key{区域状态：}重积分/几何积分当前实际覆盖什么区域；
  \item \key{方向状态：}曲线/曲面取向是否改变；
  \item \key{定义域状态：}代换、对数、平方根、分母等是否引入限制；
...
\begin{enumerate}
  ...
  \item FTC 给 $F'(x)=e^{-x^2}>0$，调用\key{局部模型}得到严格单调；
...
\begin{enumerate}
  \item \key{极限：}用可计算的邻近/有限对象定义不可直接触碰的理想对象，并比较主导尺度；
  \item \key{局部模型：}导数是最佳一阶线性化，Taylor/Hessian 是更高阶局部结构；
  \item \key{累积：}积分是“局部贡献 $\times$ 几何微元”的连续求和；
  \item \key{局部--整体：}FTC、Green、Gauss、Stokes 都把内部微分结构与边界/整体效果连接；
  \item \key{无限构造：}级数研究有限部分怎样在极限下构造无限和或函数表示；
  \item \key{动态恢复：}微分方程从局部变化规律和初值恢复整条未知轨迹。
\end{enumerate}
\end{corebox}

\section{第二层：一张对象--操作矩阵}
\begin{longtable}{L{3.1cm}L{3.8cm}L{4.0cm}L{\dimexpr\linewidth-11.07cm\relax}}
\toprule
\textbf{对象} & \textbf{局部观察} & \textbf{整体/累积} & \textbf{关键桥梁}\\
\midrule
一元函数 $f(x)$ & $f',f'',Taylor$ & $\int fdx$ & FTC / MVT\\
标量场 $f(\bm x)$ & $\nabla f,H_f$ & $\iint,\iiint$ & 梯度/极值/坐标换元\\
向量场 $\bm F$ & div / curl & circulation / flux & Green / Gauss / Stokes\\
