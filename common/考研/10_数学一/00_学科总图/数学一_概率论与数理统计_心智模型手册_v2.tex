...
\begin{longtable}{L{2.5cm}L{5.0cm}L{3.7cm}L{\dimexpr\linewidth-12.17cm\relax}}
\toprule
...
\begin{longtable}{L{2.8cm}L{4.7cm}L{4.2cm}L{\dimexpr\linewidth-12.67cm\relax}}
\toprule
...
\section{为什么 $Y=g(X)$ 不是“代入一个函数”这么简单}
...
\section{$t$：总体方差未知时，用样本方差替代会多出额外不确定性}
若总体正态，则
\[
...
\begin{longtable}{L{3.4cm}L{4.2cm}L{4.0cm}L{\dimexpr\linewidth-12.57cm\relax}}
\toprule
...
\section{单个正态总体均值：$\sigma$ 已知与未知为何对应不同标尺}
若 $\sigma$ 已知：
\[
...
\section{检验不是证明 $H_0$ 对错，而是控制错误风险的决策}
...
\section{四类典型断点如何修复}
\begin{longtable}{L{2.8cm}L{4.5cm}L{4.6cm}L{\dimexpr\linewidth-12.87cm\relax}}
\toprule
...
\section{正态总体：从模型结构到 $N,\chi^2,t,F$ 和推断全过程推演}
...
\subsection{Stage 3：抽样分布 $t$ 统计量的完整导出}
...
假设 $H_0: \mu = \mu_0$（$\sigma$ 未知）。在 $H_0$ 为真世界里，检验统计量 $T = \frac{\bar X - \mu_0}{S/\sqrt n} \sim t(n-1)$。显著性水平 $\alpha$ 下拒绝域为 $W = \{ |T| \ge t_{\alpha/2}(n-1) \}$。若观测值 $t_{obs}$ 进入拒绝域，说明看到了小概率发生的极端数据，故拒绝 $H_0$！
