...
\subsection{两个子空间怎样相加、相交？}

给定同一有限维空间中的子空间 $U,W$，它们的\kw{和空间}定义为
\[
U+W=\{u+w:u\in U,\ w\in W\}.
\]
它表示把两组方向放在一起以后总共能够生成多大的空间。交空间
\[
U\cap W
\]
则只保留两边共同拥有的方向。

若先取 $U\cap W$ 的一组基，再分别补成 $U$ 与 $W$ 的基，可以得到维数公式
\[
\boxed{
\dim(U+W)=\dim U+\dim W-\dim(U\cap W).
}
\]
这里减掉交空间，是因为公共方向在 $\dim U+\dim W$ 中被重复计算了一次。
...
\]
要找 $U\cap W$ 中的向量，不是直接比较两组系数，而是令同一个向量同时有两种表示：
\[
...
\]
解出系数以后，还必须回代得到实际公共向量；若原生成组本身相关，非零系数解仍可能只对应零向量。
\end{methodbox}
