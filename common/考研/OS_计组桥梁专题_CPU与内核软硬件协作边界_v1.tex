...
\large 从系统调用、虚拟内存、抢占、DMA 到并发原子性的统一机制图}
...
\begin{corebox}[本册总纲：不是“硬件做一半、软件做一半”，而是两层契约]
\begin{center}
...
因此最重要的问题不是“这一项到底归 CPU 还是归 OS”，而是依次追问：
\begin{enumerate}
...
\section{第一原则：Mechanism、Policy 与 State Ownership}

\subsection{硬件与内核的分工不能只记成一句口号}
“硬件提供机制，OS 提供策略”是非常好的第一层抽象，但还不够。真实系统至少有三层：
...
\renewcommand{\arraystretch}{1.25}
\setlength{\tabcolsep}{6pt}
\begin{tabularx}{\linewidth}{>{\bfseries}p{0.18\linewidth} p{0.2\linewidth} X X}
\toprule
...
\begin{eng}[现代 Linux 校准：固定 HZ 不是唯一时间模型]
...
\toprule
阶段 & Hardware / Device 职责 & Kernel / Driver 职责\\
\midrule
I/O 准备 & 暴露 MMIO/PIO 寄存器与 DMA 传输能力 & 分配 DMA Buffer/Descriptor，做物理地址 Mapping，写入命令字\\
数据传输 & DMA 控制器接管总线，在磁盘与物理内存间直接传输 & 当前线程设为 Blocked，主动调用 \code{schedule()} 避让 CPU\\
完成通知 & DMA 传输完成，向 APIC / CPU 发送硬件中断信号 & ISR 中断处理程序确认完成状态，关闭描述符，唤醒等待线程\\
后续恢复 & ——（硬件等待下一条指令） & 被唤醒线程进入 Ready 队列，由 Scheduler 未来重新分配 CPU\\
\bottomrule
\end{tabularx}
\endgroup
...
while (1) { }
\end{verbatim}
并不会主动 syscall。真正保证 OS 最终重新获得 CPU 的关键机制是\kw{可编程定时器 + 外部中断 + 特权入口}。

\begin{center}
\begin{tikzpicture}[node distance=7mm and 8mm,>=Stealth]
\tikzstyle{n}=[draw,rounded corners,align=center,minimum width=26mm,minimum height=8mm]
\node[n] (u) {A 用户态\\Running};
\node[n,right=of u] (t) {Timer Event};
\node[n,right=of t] (k) {Kernel Timer Handler};
\node[n,below=of k] (d) {need reschedule?};
\node[n,left=of d] (ret) {No: 返回 A};
\node[n,right=of d] (sch) {Yes: Scheduler};
\draw[->,thick] (u)--(t);
\draw[->,thick] (t)--(k);
\draw[->,thick] (k)--(d);
\draw[->,thick] (d)--node[above]{No}(ret);
\draw[->,thick] (d)--node[above]{Yes}(sch);
\draw[->,thick] (ret.west) -- ++(-8mm,0) |- (u.south);
\end{tikzpicture}
\end{center}
...
\begin{itemize}
  \item 无竞争：用户态原子操作直接成功，fast path；
  \item 有竞争且等待很短：可能 spin；
  \item 有竞争且需要阻塞：进入内核，加入等待队列并 schedule，slow path。
\end{itemize}
...
\small
\renewcommand{\arraystretch}{1.25}
\begin{longtable}{>{\bfseries}p{0.18\linewidth} p{0.3\linewidth} p{0.36\linewidth} p{0.12\linewidth}}
\toprule
...
&\rightarrow\text{内核接管 slow path}\rightarrow\text{修复/决策/更新状态}\rightarrow\text{重新回到高速执行}
\end{aligned}
}
\]
...
\section{超级母题：一次阻塞 read() 怎样横跨计组与 OS}
...
\subsection{第三阶段：fast path 失败}
如果 page cache hit，数据可能很快返回。
...
这条路径同时考察：系统调用入口、CPU 特权级、中断、PCB/线程状态、调度、虚拟内存保护、VFS、I/O、DMA、Cache/Page Cache 与阻塞/唤醒。以后遇到跨计组 + OS 综合题，都应该训练成这种“\kw{时间线上谁接管、状态在哪里、谁负责什么}”的推演。
\end{exam}

\section{高频误区修订：哪些话适合入门，却不适合作为最终模型}

\begingroup
\small
\renewcommand{\arraystretch}{1.25}
\begin{longtable}{>{\bfseries}p{0.32\linewidth} p{0.64\linewidth}}
\toprule
...
\section{408 做题协议：如何判断一道跨计组/OS 综合题}

\begin{method}[软硬件分工十问]
\begin{enumerate}
  ...
  \item 这条路径是 fast path 还是 slow/exception path？
...
\begin{exam}[四类最常见综合题]
\begin{itemize}
  \item \kw{系统调用/中断 + 调度}：何时进入内核？是否必然切换？
...
\section{如何嵌入现有 408 手册}

本专题建议同时承担三种角色：
\begin{enumerate}
  \item 在\kw{计算机组成原理}中，放在“中断/I/O/存储系统”之后，回答“硬件能力最终被谁使用”；
  \item 在\kw{操作系统}中，放在“控制权与系统调用/中断”之后、虚拟内存/并发/I/O 专题之前，作为五大机制的总桥梁；
  \item 在\kw{408 总手册}中，只保留 2--3 页的分工矩阵与 \code{read()} 母题，详细内容跳转到本专题册。
\end{enumerate}

\begin{bridge}[和现有专题的连接]
\begin{itemize}
  \item 《进程、线程、调度与控制权》回答：\kw{谁在运行，控制权怎样变化？}
  \item 本册回答：\kw{每次边界变化时，CPU/设备和内核究竟怎样接力？}
  \item 《信号量与 PV》回答：\kw{多个执行流怎样把共享约束编码成原子与等待机制？}
  \item 后续虚拟内存专题回答：\kw{地址抽象如何用 MMU + page table + fault handler 完成？}
\end{itemize}
\end{bridge}

\section*{工程校准资料}
\addcontentsline{toc}{section}{工程校准资料}
以下资料用于校准“现代 Linux/x86 实现”，不作为 408 必背 API：
\begin{itemize}
  ...
  \item Linux futex manual：用户态原子 fast path 与内核 wait/wake slow path。\\
...
\end{center}
真正的系统能力来自二者之间稳定而清晰的契约，而不是任何一方单独完成全部工作。
\end{corebox}
