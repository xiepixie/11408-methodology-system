...
\begin{corebox}[定位：PV 不是口诀，而是约束映射与语义建模]
信号量/PV 题不从套模板开始，而从共享状态、等待条件和不变量推导信号量：
\[
\boxed{
\text{执行流}
\to\text{交错}
...
\item \kw{许可计数（Permit Count）}：信号量整型变量 $S$ 的当前可用数值（如 $S=1$ 表示 1 个互斥进入名额，$S=N$ 表示 $N$ 个缓冲区空位额度）。
\item \kw{约束映射（Constraint Mapping）}：将现实物理/业务规则推演翻译为 PV 逻辑的思考建模过程。
\item \kw{PV 伪代码（PV Pseudocode）}：答题卡上最终写出的 \code{P(S)} / \code{V(S)} 程序代码片段。
\end{itemize}
\end{corebox}
