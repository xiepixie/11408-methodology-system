...
因此可以抽象为：
\[
\boxed{\text{fd}\rightarrow\text{打开文件描述（OFD）}\rightarrow\text{文件系统对象}}
\]
...
\subsection{第二层：打开文件描述代表“打开实例”}

POSIX 称这一抽象为打开文件描述（open file description，OFD）；Linux 通常用 \code{struct file} 实现相应的打开实例状态。

这一层保存与“这一次打开”相关的状态，例如当前文件偏移、某些状态标志以及指向具体文件对象的引用。

重要语义：
\begin{itemize}
\item 两次独立 \code{open()} 同一路径通常得到两个独立的 OFD，因此文件偏移可以独立；
\item \code{dup()} 产生新的 fd，但指向同一个 OFD，因此共享文件偏移；
\item \code{fork()} 后父子各自拥有 fd 表，但对应 fd 通常仍引用同一个 OFD，因此共享该打开实例的文件偏移和相关状态。
\end{itemize}
...
\item fd 指向打开文件描述（OFD）；在 Linux 中通常对应 \code{struct file}，其中保存 offset 与对象引用。
\item VFS/具体文件系统沿 dentry/inode、缓存和块映射路径处理请求。
\item 数据若已经在页缓存：走快路径，复制并返回，A 不必阻塞。
...
\begin{exam}[一题串起的考点]
一次阻塞 \code{read()} 可以同时考：系统调用、用户/内核态、fd、进程资源、运行/阻塞/就绪状态、调度、上下文切换、设备中断、DMA、文件缓存与唤醒。
\end{exam}
...
\item POSIX / The Open Group：\code{fork()}、\code{open()}、\code{dup()}、\code{exec()} 对文件描述符与打开文件描述的规范语义。
