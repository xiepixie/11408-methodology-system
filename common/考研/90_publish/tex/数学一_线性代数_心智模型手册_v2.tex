...
\[
\|x\|=\sqrt{\langle x,x\rangle},\qquad x\perp y\iff \langle x,y\rangle=0.
\]
...
先看 $AA^*=|A|I$。若 $|A|\neq0$，直接化成逆矩阵问题；若 $|A|=0$，再看 $A^*$ 的元素本质上是 $(n-1)$ 阶余子式。这样 rank 结论是从结构推出，而不是孤立口诀。
\end{examBox}
...
\begin{enumerate}
  ...
  \item 对每个参数区间/特殊点比较 $r(A)$ 与 $r(A|b)$；
...
\section{为什么要解 $\det(\lambda I-A)=0$}
条件
\[
(A-\lambda I)v=0
\]
...
\[
\boxed{\text{safe elimination}\to\text{rank breakpoints}\to r(A)\text{ vs }r(A|b)\to\text{solution type}\to\text{specific solution}}.
\]
这是最值得长期固化成操作习惯的一类题。
