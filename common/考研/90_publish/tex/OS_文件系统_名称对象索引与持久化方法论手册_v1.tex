...
\large 从 Path、fd、inode、块映射到 Page Cache、Journaling 与 Crash Consistency 的统一方法论}
...
\begin{corebox}[本册总纲：文件系统不是“inode 知识点集合”，而是一个持久对象世界]
...
Access & 打开后可持续读写 & fd、open file description、offset \\
...
\begin{itemize}
  ...
  \item \kw{本专题}回答：\code{/a/b/c} 或 \code{fd=3} 怎样变成文件对象、文件偏移与存储位置，以及这些持久元数据怎样保持一致。
\end{itemize}
\end{bridge}
...
\begin{tikzpicture}[x=3.15cm,y=1.85cm]
  ...
  \node[fsnodeg] (cache) at (3,-1) {Cached Page\\[-1pt]\scriptsize page cache};
...
\section{第 2 章：目录、路径与链接——Name $\to$ Object}
...
\end{tikzpicture}
\captionof{figure}{路径解析：先沿目录逐级推进，dcache 是可选 fast path，而不是磁盘目录项本身。}
\end{center}
...
\begin{exam}[2026 408 范围校准]
公开的大纲转载版本明确列出：文件元数据与 inode、文件操作、文件保护、逻辑/物理结构、目录、硬/软链接、文件系统全局 layout、空闲空间管理、VFS 与 mounting。故本册把这些作为 \kw{408 Core}；Journaling、extents、dcache/XArray 等作为机制深化或工程校准，而不是替代教材模型。
\end{exam}

\section{Part II：进程怎样引用已经找到的文件}

\section{第 4 章：fd、Open File Description 与 inode}
...
于是必须增加一层：
\[
\boxed{\text{fd}\arr\text{Open File Description}\arr\text{File Object/inode}}
\]
POSIX 语义中，每次成功 \code{open()} 创建一个新的 open file description；文件 offset 和 file status flags 属于它，而 fd 只是进程内的句柄。
...
\end{tikzpicture}
\captionof{figure}{dup 的本质不是再次 open，而是增加一个 fd 句柄指向同一个 open file description。}
\end{center}
...
一个 inode 可以同时被多个 open file description 引用。\kw{文件对象身份}和\kw{打开实例状态}必须分开，否则无法解释“同一文件独立打开两次 offset 不共享”。
\end{warn}
...
\begin{itemize}
  ...
  \item \kw{Open/reference lifetime}：运行时还有没有 open file description / 内核引用使对象仍被使用？
\end{itemize}
真正安全的高层结论是：
\[
...
\]
408 常见机制包括访问权限、用户/组/其他（owner/group/other）读写执行位等。目录上的 read/write/execute 与普通文件语义不同：目录 execute 更接近“允许搜索/穿越”，write 与创建/删除目录项相关。
...
\section{第 7 章：File Offset $\to$ Logical Block}
设文件系统块大小为 $B$，字节偏移为 $x$：
\[
...
\end{itemize}
但如果索引块已缓存，次数会下降。因此任何 I/O 计算都先写：\kw{inode 是否在内存？索引块是否缓存？目录块是否缓存？Page Cache 是否命中？}

\begin{eng}[现代 ext4：extent 不是“12+1+1+1”的简单延长]
经典 direct/single/double/triple 模型非常适合 408 计算题；现代 ext4 可使用 extent tree，把“连续逻辑范围 $\to$ 连续物理范围”作为映射单位。Linux ext4 文档明确区分 inode 的 extent 格式等结构。工程校准用于理解“现代文件系统为何减少大文件映射元数据”，但不替代经典计算模型。
\end{eng}
...
\]
其中 \code{open()} 的 POSIX 语义包括创建新的 open file description，并让一个 fd 指向它；常规文件初始 offset 通常从文件开头开始（除特定 flag 语义）。
...
\subsection{现代 buffered read：先问 Page Cache}
...
\begin{tikzpicture}[x=3.0cm,y=1.55cm]
  ...
  \node[fsnode] (pc) at (3,0) {Page Cache?};
  ...
  \draw[mainflow] (pc) -- node[labelnote,right=3pt]{fast path} (hit);
...
常见普通写路径：
\[
\text{User Data}\arr\text{Page Cache}\arr\text{Dirty}
\]
...
\begin{itemize}
  ...
  \item open file description 的 offset。
\end{itemize}
...
\subsection{模型 B：访问第 $x$ 字节属于哪一级索引}
先：
\[
...
Page Cache vs block map & 先求 PBN 才能查缓存 & buffered file cache 更自然按 file mapping + page index 组织 \\
...
\section{附录 A：408 Core / Mechanism / Engineering 分层清单}
\begin{center}
\footnotesize
\begin{tabularx}{\linewidth}{>{\bfseries}p{0.16\linewidth} X X X}
\toprule
主题 & 408 Core & 机制深化 & 工程校准 \\
\midrule
...
\section{附录 B：资料校准与来源}
\begingroup\Urlmuskip=0mu plus 3mu\relax
本册正文以 408 教材模型为主，以下公开资料用于校准术语和现代系统边界：
\begin{itemize}
  ...
  \item 2026 408 大纲公开转载（非教育部原始规范页），用于范围核对：\url{https://kaoyan.xdf.cn/202511/15003718.html}
\end{itemize}
\endgroup

\begin{history}[关于本册使用的用户积累]
本册吸收并重构了既有笔记中的：inode/目录项/超级块、文件描述符与打开文件表、硬软链接、连续/链接/索引分配、空闲空间管理、\code{open/read/write/close} 生命周期、Page Cache 与 crash/journaling 等内容。对于“offset 所属层级、dentry 含义、inode 打开计数、ext4 索引方式、Page Cache 查找键”等存在教材模型与工程实现差异的地方，正文采用“408 Core + 严格语义 + 工程校准”三层表述。
\end{history}
