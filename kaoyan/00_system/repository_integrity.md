...
`90_publish/` 按科目组织分类子目录（`math1/`, `english1/`, `408/`, `system/`）。两个不同 `.tex` 如果 stem 相同且在同一分类下，都会写成同一个 PDF，因此直接报错。
...
### E-EXAM-SOLUTION-*｜408 Derived Solution Layer 结构回归

只要某年度已经建立 `archives/408/<year>年真题/solutions/` 且存在 `qNN.md`，`check` 就把它视为正在维护的 Derived Solution Layer。生命周期分两级：

- **partial**：已有部分 `qNN.md`，但尚未创建年度 `README.md / solution_review.md`，也尚未达到 Q01～Q47 全覆盖；此时只检查已经存在的题级机器事实，允许继续增量写题；
- **complete**：出现年度 `README.md`、`solution_review.md`，或已经达到 Q01～Q47 全覆盖中的任一条件，即视为进入整年完成态；从这一刻开始强制 Coverage=47/47，同时要求 README + solution review 齐全。

无论 partial / complete，已有题解都必须满足：

- Frontmatter 必填字段完整，Q1～Q40 的 `answer` 必须为 A/B/C/D；
- `exam_id / question_id / question_number / status` 使用统一稳定值；
- `subject` 必须符合 408 Exam Profile 及年度 routing override；
- Q1～Q40 `answer` 必须与年度全景索引一致；
- `source_exam / legacy_reference` 目标真实存在；
- Q1～Q40 与 Q41～Q47 的固定 H2 必须完整、唯一、非空且顺序一致；额外细分只能使用 H3；
- `Model Anchor` 必须包含“题目信号 / 第一动作”；
- H1 必须使用 `408-YYYY-QNN`。
...
这是机器可确定的架构回归，因此属于硬错误。

### E-GENERATOR-SSOT｜生成器单源事实与年份/Profile 一致性

所有生成器、构建工具与维护脚本（如 `progress` 渲染器、真题构建脚本、元数据核验工具等）必须以当前文件系统真实状态与 Canonical `exam_profiles/*.json` 为唯一真相源（Single Source of Truth），严禁将旧迁移时代的硬编码快照（Snapshot）、过时年份范围或已废弃假设写回仓库：

- **动态扫描**：任何自动生成的清单与汇总（如 `PROGRESS.md`）必须通过动态扫描真实资产计算生成，不得依赖静态硬编码快照；
- **配置直连**：真题库年份跨度、试卷结构与学科题号路由必须直连 `00_system/exam_profiles/`，工具脚本不得自行定义或硬编码冲突的结构假设；
- **无损保护**：生成与构建操作默认保护已有 Canonical 正文与元数据，不得发生覆盖退化或生成态污染。
...
- Topic / Bridge / Integration Landing Page 超过 250 行时提示：检查是否把深度正文留在 README。
...
本项目不把 `infra/scripts/compile_tex.py` 直接当成业务发布入口。原因不是重新实现 LaTeX 编译，而是共享脚本只拥有 Domain-Agnostic 编译机制，不知道当前 `.tex` 是否真的是 Kaoyan 的 Canonical Handbook，也不会替本项目检查 Owner、stem 冲突和 Landing Page。

`publish` 只发布 Topic / Bridge / Integration 的 Canonical 深度正文。调用共享编译器之前必须确认：

1. 目标 `.tex` 位于 `kaoyan/` 内；
...
调用共享编译器时，项目安全入口会统一注入当前 Infra/Kaoyan TeX 搜索根；新建 Canonical Handbook 使用正式 `ipara-handbook` Class，既有 Prototype 文档也可继续通过同名兼容宏包稳定编译，不需要维护脆弱的多层相对路径。正式 Core/Class 架构与 Infra regression Gate 由全局 [`infra/latex/README.md`](../../infra/latex/README.md) 拥有；`kaoyan/ipara-handbook.sty` 只作为存量 Compatibility Surface，本仓库的发布/搜索路径安全仍由本文件拥有。共享编译器只拥有通用 TeX 搜索与编译机制，Kaoyan 根目录和 `90_publish/` 由本项目安全入口显式注入。
...
如果修改了 Handbook Forward Template 或其 LaTeX 合同，再运行：

```bash
python3 00_system/check_handbook_template.py
```
