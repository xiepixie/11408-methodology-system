...
需要长解释、推导、机制和例题的 Handbook 继续使用。一个专题目录同时也是该专题的训练入口：

```text
<Handbook Directory>/
├── README.md              # Landing Page + 训练导航
├── <Handbook>.tex         # Canonical deep body：机制与世界模型
├── <训练主题 A>.md        # 可选，例如：反函数.md
├── <训练主题 B>.md        # 可选，例如：周期性.md
├── <训练主题 C>.md        # 可选，例如：单调性.md
└── assets/                # 可选
```
...
但知识 Owner 仍保持唯一：数学定义、定理、机制、证明与边界由 Canonical `.tex` 拥有；训练 Markdown 通过链接调用，不复制第二套完整机制正文。
...
它们的 README 不得承载完整定义链、长推导、完整机制正文、整套例题或复制 `.tex`。若同目录存在训练 Markdown，README 应按学生使用顺序提供训练导航，但不复制训练正文。
...
专题训练不是知识点目录，也不是题目答案仓库。它负责把大量具体题压缩为少量稳定的问题族，并把与这个问题族紧密绑定的做题动作放在同一处。

一份训练 Markdown 可以自然包含：

- **模型锚点**：调用 Canonical Handbook 的哪一个机制；
- **母题表示**：这类题真正的对象、关系与目标；
- **变式轴**：题目换皮时通常改变哪些条件；
- **关键决策点**：什么信息真正决定路径；
- **局部规则**：看到什么、第一步做什么、怎样检查、何时退出；
- **局部技巧**：只在这个问题族里高价值的换元、构造或速算；
- **代表题**：真题、错题、陌生题的索引与验证证据。

训练文件默认与最相关的 Topic / Bridge / Integration 共置，并按内容命名。拆分粒度服从**检索意图与增长压力**，不服从教材小节数量：如果学习者会独立检索“反函数”“周期性”“单调性”，就可以分别建 `反函数.md`、`周期性.md`、`单调性.md`；若两个问题总是共享同一组母题与决策点，可以先合并，等内容增长后再拆。不要为了目录整齐把每个知识点都变成一个文件。

如果一道题必须组合多个 Owner 才构成完整过程，则训练内容可以放到相应 Integration 目录下；不要为了题型导航再建立一棵平行学科目录。

Handbook 可以保留少量能跑通机制的**母例**；训练 Markdown 负责把机制扩展成题目家族。母例的目标是解释，母题的目标是迁移、识别与执行。
...
对同目录训练 Markdown，这四项统一压成“**触发信号—第一动作—检查与退出**”三元组；局部规则默认不分配学科级稳定编号，只有经过证据验证并晋升 Subject Rules 后才分配编号。具体格式见 [`topic_practice_writing_spec.md`](topic_practice_writing_spec.md)。
...
## 10. AI-readable，而不是 machine-schema
