...
| Complexity / Operation Cost Vector | `30_408/10_数据结构/README.md` Atlas Foundation | 全部数据结构 Topic | 是全科学术语/度量语言，不再建立独立 complexity Topic |
| Frontier Traversal Model | `30_408/10_数据结构/50_科内桥梁/DS-B01_FrontierTraversal/README.md` | Tree、Graph、Stack、Queue | Tree/Graph 各自拥有结构语义，Bridge 只拥有 frontier 展开接口 |
| Heap / Priority Queue | `30_408/10_数据结构/05_Heap与优先队列/README.md` | Heap Sort、Prim、Dijkstra | 排序和图算法只调用优先级操作 |
| Union-Find | `30_408/10_数据结构/06_UnionFind与集合划分/README.md` | Kruskal、动态连通 | 图算法只调用 Find/Union |
| Ordered Index Trade-off | `30_408/10_数据结构/50_科内桥梁/DS-B02_IndexStrategy与Workload/README.md` | Binary Search、BST、AVL/RB、B/B+、Hash | 各 Topic 拥有机制，Bridge 拥有 workload/cost 比较 |
| Incremental Sequence Scan / Interval State | `30_408/10_数据结构/70_算法扩展/DS-A01_序列扫描与区间状态/DS-A01_序列扫描与区间状态_方法论手册.tex` | Prefix/Diff、Window、Two Pointers、Matrix Boundary | Extension Topic；二分归 DS09，单调栈/队列归 DS02 |
| Decision-space Search / Backtracking | `30_408/10_数据结构/70_算法扩展/DS-A02_状态空间搜索与回溯/DS-A02_状态空间搜索与回溯_方法论手册.tex` | Permutation/Combination/Subset、CSP、Pruning | Extension Topic；树/图 DFS 的遍历机制仍归 DS04/07 |
| Dynamic Programming / State Compression | `30_408/10_数据结构/70_算法扩展/DS-A03_动态规划与状态压缩/DS-A03_动态规划与状态压缩_方法论手册.tex` | Knapsack、Grid、Edit Distance、Stock、LIS/LCS | Extension Topic；拥有状态充分性与依赖，不复制结构机制 |
| Greedy Choice / Exchange Proof | `30_408/10_数据结构/70_算法扩展/DS-A04_贪心选择与交换证明/DS-A04_贪心选择与交换证明_方法论手册.tex` | Interval Scheduling、Jump Frontier、Cut/Stays-ahead | Extension Topic；堆/并查集/图算法只提供或接收接口 |
...
| Page Cache | `30_408/30_操作系统/60_科内桥梁/OS-B04_VMFileIO/README.md` | VM、文件系统、OS I/O、read Integration | File 拥有内容身份，VM 拥有驻留映射，I/O 拥有 miss/writeback；Hardware Cache ≠ Page Cache |
...
正式 OS Subject 入口见 [408 / 操作系统导航](../30_408/30_操作系统/README.md)。
...
| B09 协方差矩阵与二次型 | `10_数学一/50_桥梁专题/B09_协方差矩阵与二次型_随机波动的方向结构/README.md` | 拥有 `Var(a^T X)=a^TΣa`、协方差正半定、`Cov(AX)=AΣA^T` 与主波动方向翻译；概率 Topic04/05 和线代 Topic05/06 保留本体机制 |
| B10 样本中心化与正交投影 | `10_数学一/50_桥梁专题/B10_样本中心化与正交投影_自由度与抽样分布/README.md` | 拥有均值投影、残差子空间、`n-1` 自由度与正态下正交分量独立的跨科接口；概率 Topic07 保留抽样分布机制 |
...
| 线性代数跨训练专题做题控制 | `10_数学一/90_学科做题规则/线性代数.md` | 当前没有仍需由 Subject Rules 单独拥有的活跃规则；单一问题族的识别、路径、速算与校验均由对应 Topic / Bridge Practice 拥有。未来只有跨多个训练专题反复验证、且无法由单一 Practice 完整拥有的控制才晋升到此处 |
| 概率统计学科母模型与 Topic 地图 | `10_数学一/30_概率论/README.md` | Atlas 只拥有母模型、Topic 地图和跨册压缩 |
| 事件、样本空间与概率公理 | `10_数学一/30_概率论/01_随机世界_事件与概率/随机世界_事件与概率.tex` | Canonical Topic01；README 只做 Landing。拥有事件空间与概率机制，不拥有条件更新与随机变量分布 |
| 条件概率、独立性、全概率与 Bayes | `10_数学一/30_概率论/02_条件概率_独立性与Bayes/条件概率_独立性与Bayes.tex` | Canonical Topic02；README 只做 Landing。拥有条件更新与独立性机制，连续条件分布的计算接口交给 Topic04 |
| 随机变量与一维分布 | `10_数学一/30_概率论/03_随机变量与一维分布/随机变量与一维分布.tex` | Canonical Topic03；README 只做 Landing。拥有观察函数、CDF、PMF、密度与一维分布生成语义 |
| 联合、条件、边缘与变量变换 | `10_数学一/30_概率论/04_联合分布_条件分布与变换/联合分布_条件分布与变换.tex` | Canonical Topic04；README 只做 Landing。拥有联合支撑、条件/边缘对象与概率质量重组机制 |
| 期望、方差、协方差与条件摘要 | `10_数学一/30_概率论/05_数字特征与依赖摘要/数字特征与依赖摘要.tex` | Canonical Topic05；README 只做 Landing。只拥有摘要机制，不拥有完整分布 |
| 大数定律与中心极限定理 | `10_数学一/30_概率论/06_大数定律与中心极限定理/大数定律与中心极限定理.tex` | Canonical Topic06；README 只做 Landing。区分稳定位置与标准化波动 |
| 总体、样本、统计量与抽样分布 | `10_数学一/30_概率论/07_总体样本与抽样分布/总体样本与抽样分布.tex` | Canonical Topic07；README 只做 Landing。拥有统计随机标尺，不作推断决策 |
| 参数估计、置信区间与假设检验 | `10_数学一/30_概率论/08_参数估计与假设检验/参数估计与假设检验.tex` | Canonical Topic08；README 只做 Landing。用抽样分布校准逆向推断 |
| 概率统计跨训练专题做题控制 | `10_数学一/90_学科做题规则/概率统计.md` | 只拥有跨多个概率训练专题仍需统一维护的交接与证据层级控制；单一问题族的识别、路径、执行与检查由对应 Topic / Bridge Practice 拥有 |
