...
\begin{itemize}
  ...
  \item 单一积分问题族的题目信号、预处理顺序与停止条件进入同目录训练 Markdown；跨多个训练专题反复成立并经证据验证的控制才进入《高等数学做题规则》。
\end{itemize}
...
\int f(x)\,dx=F(x)+C.
\]
若 $F,G$ 在同一\textbf{区间}上都是原函数，则 $(F-G)'=0$，所以 $F-G=C$。区间若被断开，各连通分支可以有不同常数；这正是 $\int dx/x=\ln\lvert x\rvert+C$ 不能跨过 $0$ 统一成一个单常数表达的原因。
...
\[
\int\frac{u'}u\,dx=\ln\lvert u\rvert+C,
\qquad
...
\]
前者对应 $x=a\sinh t$，因为 $\cosh^2t-\sinh^2t=1$；后者在正分支对应 $x=a\cosh t$。若要同时覆盖 $x>a$ 与 $x<-a$ 两个连通分支，应回到
\[
\ln\left\lvert x+\sqrt{x^2-a^2}\right\rvert+C
\]
并分别审计定义域。类似地，$1/(a^2-x^2)$ 在 $\lvert x\rvert<a$ 可识别为 $\operatorname{artanh}(x/a)$ 的导数。双曲函数对象与分支归 Topic01，导数归 Topic03；本册只拥有“这些导数怎样作为原函数被识别，以及何时用双曲参数化降低积分复杂度”。具体训练见《双曲型根式与特殊原函数》。
...
\section{微元建模：局部贡献如何进入一元累积}

微元建模的核心是一元累积机制的物理/几何实例化：把整体连续介质沿某一自变量切分成互不重叠的薄层，写出带微分因子的局部贡献 $dQ=q(x)\,dx$，并在合法区间上累加。面积、弧长、旋转体、功、压力、形心与相关变化率的完整跨模块 Composition 及防重复覆盖协议由 \kw{H-I01（微积分建模：从局部微元到整体量）} 唯一持有；本册仅提供一元累积机制的接入规范。

\subsection{面积、弧长与旋转体资格}

竖直切片给 $dA=[\text{上函数}-\text{下函数}]dx$；水平切片则用 $dy$。弧长来自微小线段
\[
ds=\sqrt{dx^2+dy^2}=\sqrt{1+{(y')}^2}\,dx.
\]
绕坐标轴旋转时，垂直于轴切片得到圆盘/垫圈（累加截面积），平行于轴切片得到圆柱壳（累加周长乘高）：
\[
V=\pi\int(R^2-r^2)\,dx,
\qquad
...
\begin{warnbox}{圆柱壳法的资格限制与跨轴审计}
普通壳层公式 $V=2\pi\int r(x)h(x)\,dx$ 仅在\textbf{积分区域完全位于旋转轴的一侧}时成立。若区域跨越旋转轴，轴两侧到旋转轴距离相同的点会旋转生成同一个圆柱壳；此时机械将两侧全部积分会造成\textbf{重复覆盖}。遇到跨轴区域，必须改按物理半径 $r\ge0$ 对高度做并集合并，或转交 \kw{H-I01} 执行完整的覆盖重数审计。
\end{warnbox}

\subsection{功、压力与形心接口}

变力做功是 $dW=F(x)\,dx$；抽水做功的微元是“微小重量 $\times$ 提升距离”；静水压力的微元是“压强 $\rho gh\times$ 条带面积”。形心/质心的统一思想是先累加质量与一阶矩，再取比值：
\[
\bar x=\frac{\int x\,dm}{\int dm},
\qquad
\bar y=\frac{\int y\,dm}{\int dm}.
\]
任何应用建模都应做单位检查：$dA$、$ds$、$dV$、$dW$ 的量纲必须与目标物理量一致；若单位不对，说明微元翻译或变量选择发生了第一偏离。完整多对象组合与质心势能压缩模型见 \kw{H-I01}。
...
绝对收敛先检查 $\int\lvert f\rvert$。若绝对收敛失败但振荡抵消持续存在，可用 Dirichlet/Abel 型结构。Dirichlet 的典型接口是：振荡因子拥有一致有界的原函数，权重单调趋于零；Abel 则用一个已收敛的反常积分配合有界单调权重。分部积分把尾部压成边界项与可控积分。振荡并不自动收敛，必须写出“谁提供抵消、谁提供衰减”。
