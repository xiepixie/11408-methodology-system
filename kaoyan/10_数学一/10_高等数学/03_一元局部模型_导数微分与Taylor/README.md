...
- [导数定义与分段点](导数定义与分段点.md) —— 新定义、分段点、左右导数、点导数与导函数极限的对象分流；
- [求导法则与特殊函数](求导法则与特殊函数.md) —— 用“局部增益代数”重建和差/乘积/商/复合/反函数/隐函数/参数方程求导，并从幂、指数、三角、双曲与反函数母族恢复常用导数和有限 Taylor 式；
- [导函数的介值性与极限边界](导函数的介值性与极限边界.md) —— 导数极限定理、Darboux 介值性、导函数不允许的第一类/单向无穷间断与可发生的振荡间断；
- [高阶导数结构识别](高阶导数结构识别.md) —— 循环型、有限消失型、可拆型、Leibniz、低阶恒等式递推与 Taylor 系数法的结构选择。
...
I-06 与 I-02 中相关片段已完成逐文件 Source Diff / Owner Diff。本轮补齐旧稿中容易被“绝对值点分左右”一句话吞掉的三种不同结构：$f(x)\lvert x-a\rvert$、$\lvert f(x)\rvert$、$f(\lvert x\rvert)$，并把曲率圆重写为“曲线上目标点 + 沿凹侧单位法向移动一个曲率半径”的生成模型。正文与同目录训练文档尚需使用者审阅和陌生题验证；跨专题候选规则继续按证据晋升流程处理，当前不标记为“已采用”。
