...
- [高阶导数结构识别](高阶导数结构识别.md) —— 循环型、有限消失型、可拆型、Leibniz 与 Taylor 系数法的结构选择。
...
I-06 与 I-02 中相关片段已完成逐文件 Source Diff / Owner Diff。本轮补齐旧稿中容易被“绝对值点分左右”一句话吞掉的三种不同结构：$f(x)|x-a|$、$|f(x)|$、$f(|x|)$，并把曲率圆重写为“曲线上目标点 + 沿凹侧单位法向移动一个曲率半径”的生成模型。正文与同目录训练文档尚需使用者审阅和陌生题验证；跨专题候选规则继续按证据晋升流程处理，当前不标记为“已采用”。
