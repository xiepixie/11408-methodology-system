...
\paragraph{乘积型 $g(x)=f(x)|x-a|$.}
若 $f$ 在 $a$ 连续，则 $g(a)=0$，并且
\[
\frac{g(a+h)-g(a)}h=f(a+h)\frac{|h|}{h}.
\]
...
\paragraph{外复合型 $g(x)=|f(x)|$.}
若 $f$ 在 $a$ 可导且 $f(a)=0$，则
\[
f(a+h)=f'(a)h+o(h).
\]
故右、左差商的主部是 $|f'(a)|$ 与 $-|f'(a)|$，于是
\[
\boxed{|f|\text{ 在 }a\text{ 可导}\iff f'(a)=0}.
\]
这说明“内函数在零点”只是候选；真正决定外层尖角是否被压平的是内函数的一阶零点是否也消失。

\paragraph{内复合型 $g(x)=f(|x|)$.}
若 $f$ 在 $0$ 右侧可导，则
\[
...
所以
\[
\boxed{f(|x|)\text{ 在 }0\text{ 可导}\iff f'_+(0)=0}.
\]
其本质是 $|x|$ 把同一条右侧分支反射到左侧，要求反射后的两条一阶斜率能够接缝。
