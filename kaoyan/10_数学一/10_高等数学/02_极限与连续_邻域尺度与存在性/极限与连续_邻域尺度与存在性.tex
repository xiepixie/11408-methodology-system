...
\input{assets/src/极限_输入邻域与输出控制.tex}
\end{tikzpicture}
\end{center}
...
% 第一排：主干认知链 (Approach -> Allowed Set -> Output Control -> Agreement)
...
\node[box] (ctrl) at (8.4,0)  {\textbf{3. 输出控制}\\[2pt]\scriptsize 主导尺度 / 夹逼 $|f-L|<\varepsilon$};
...
在正底数、指数趋于 $+\infty$ 的合法语境下，$0^\infty$ 通常趋于 $0$，不是未定式；若底数符号或定义域不稳定，必须先回到专题01检查对象是否合法。
...
\begin{enumerate}
  ...
  \]
  它的价值不是“看到根式就乘共轭”，而是把隐藏在平方根里的\textbf{差尺度}暴露出来。趋于负无穷时还必须单独处理 $\sqrt{x^2}=|x|$ 的符号。
...
若进一步有\kw{一致收敛}，即对每个 $\varepsilon>0$ 都能找到一个与 $x$ 无关的统一 $N$，使全体 $x$ 同时满足 $|f_n(x)-f(x)|<\varepsilon$，那么连续性才会传给极限函数。因此在连续函数列的语境中：\textbf{若点态极限已经不连续，则这列函数不可能在该集合上一致收敛。}
...
\node[box]     (filt) at (0,-2.3)   {\textbf{6. 不动点筛选}\\[2pt]\scriptsize 排除伪根 / 二周期};
...
本册规范正文的责任到“识别当前在处理哪一种极限机制、知道缺什么信息、知道何时转交”即停止。完整题型树、局部技巧和考场状态机由同目录《极限计算》《连续与间断》《递推数列》承担。
...
进入本册后只先确认三件事：
\begin{enumerate}
  \item \textbf{允许怎样趋近？} 定义域是否迫使拆左右、单侧或子列？
  \item \textbf{极限是否已经有存在性资格？} 是否能立即找到路径冲突、振荡或无界变号？
...
按考纲查漏，本册覆盖：数列与函数极限、左右极限、极限性质与运算、无穷小/无穷大及阶比较、七类未定式判别与标准化、斯托尔茨差分比接口、夹逼与单调有界准则、连续与间断、初等函数连续性。
...
\begin{enumerate}
  ...
  \item 结论中出现的每个极限，题设或前面的推理是否已经保证它存在？比较两个函数时，是否已经取得共同合法去心邻域？
