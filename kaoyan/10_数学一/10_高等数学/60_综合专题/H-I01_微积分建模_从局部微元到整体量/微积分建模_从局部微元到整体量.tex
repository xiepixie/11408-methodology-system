...
\colorlet{themecurve}{blue!70!black}\colorlet{themealert}{red!70!black}\colorlet{themeamber}{orange!80!black}\colorlet{themegreen}{green!60!black}
\begin{document}
...
\subsection*{2.\ 质量、质心与转动惯量：先统一成质量微元}
线、面、体上的公式表面不同，真正共同的对象只有质量微元
\[
\boxed{dm=\rho\,d\mu},
\]
其中 $d\mu$ 按载体分别取 $ds,dA,dS,dV$。于是
\[
M=\int dm,
\qquad
\bar{\mathbf r}=\frac1M\int \mathbf r\,dm,
\qquad
I_L=\int d(P,L)^2\,dm.
\]
质心是位置向量的质量加权平均；绕轴 $L$ 的转动惯量则是“到轴距离平方”的质量加权总和。不同几何载体只改变 $d\mu$，不改变这两个生成机制。

这也给出两个高价值检查：若载体与密度共同关于某个平面/轴对称，质心必须落在相应对称集合上；转动惯量的量纲应为 $[\text{质量}][\text{长度}]^2$。如果题目只给形心，等价于取常密度；密度不均匀时不能继续把几何中心当成质心。

\begin{center}
\input{assets/src/micro_element_matrix_body.tikz}
\end{center}

\subsection*{3.\ 极坐标与参数曲线的几何量切片}
极坐标下计算公共区域面积时，必须先解出交点确定分段角 $\theta_0$，再沿极径射线判断各段的上/外边界：
\[
A = \int_0^{\theta_0}\frac12 r_2^2(\theta)\,d\theta + \int_{\theta_0}^{\pi/2}\frac12 r_1^2(\theta)\,d\theta.
\]

\begin{center}
\input{assets/src/polar_boundary_switch_body.tikz}
\end{center}

\subsection*{4.\ 旋转体：选择的是切片方向，不是背公式}
同一平面区域绕轴旋转时有两种局部化：垂直于轴切片得到圆盘/垫圈，平行于轴切片得到圆柱壳。
若已知 $y=f(x)$ 且绕 $y$ 轴，比较两条路线：反解 $x=g(y)$ 后用圆盘，或保留 $x$ 用壳元
\[
dV=2\pi\,\underbrace{\lvert x-x_{\rm axis}\rvert}_{\text{半径}}
...
\[
2\pi\int \lvert x-x_{\rm axis}\rverth(x)\,dx
\]
...
\begin{center}
\input{assets/src/cross_axis_shell_body.tikz}
\end{center}
...
dV=2\pi r(x,y)\,dA,
\qquad
r(x,y)=\frac{\lvert ax+by+c\rvert}{\sqrt{a^2+b^2}}.
\]
于是
\[
...
\subsection*{5.\ 相关变化率：保持动态对象直到求导完成}
...
\begin{center}
\input{assets/src/pumping_work_depth_body.tikz}
\end{center}
...
\]
“深度”不是任意坐标值，“移动距离”也不是切片位置；二者都要从物理对象重新定义。��应按“半径集合”理解：一个垂直于轴的线段若跨过旋转轴，旋转后内半径是 $0$，形成圆盘；不能把上下边界到轴的距离机械做平方差。
...
\subsection*{7.\ 引力：先写质量微元产生的向量贡献}
设待研究质点质量为 $m_0$、位置为 $\mathbf r_0$，分布物体上的质量微元 $dm$ 位于 $\mathbf r$。微元对质点的万有引力方向指向 $\mathbf r$，因此
\[
\boxed{
d\mathbf F
=Gm_0\frac{\mathbf r-\mathbf r_0}
{\lVert\mathbf r-\mathbf r_0\rVert^3}\,dm.}
\]
总引力不是把距离先平均再代平方反比，而是逐点累加向量：
\[
\mathbf F=\int d\mathbf F.
\]
若物体关于经过 $\mathbf r_0$ 的某个轴/平面对称，先用成对质量元消去横向分量，再只积分可能保留的轴向分量；这通常比一开始写三个坐标分量更稳定。

引力题仍服从同一建模母流程：\textbf{对象与密度 $\to dm$ $\to$ 单个微元的方向和大小 $\to$ 对称降维 $\to$ 积分 $\to$ 方向/量纲复核}。必须额外审计奇点：若观察点位于连续质量分布内部或积分载体穿过 $\mathbf r_0$，核函数 $1/\lVert\mathbf r-\mathbf r_0\rVert^2$ 出现奇性，不能无条件按普通有界连续积分处理。

\subsection*{8.\ 最小组合例：变密度细杆}
