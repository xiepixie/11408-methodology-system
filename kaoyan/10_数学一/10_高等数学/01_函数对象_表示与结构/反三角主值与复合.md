...
是**函数值的乘法逆元（倒数）**，不是映射的反函数。为避免歧义，本手册不使用 $(\cos x)^{-1}$、$(\sin x)^{-1}$ 或 $(\tan x)^{-1}$ 表示倒数。

- $\sec x \ne \arccos x$
- $\csc x \ne \arcsin x$
- $\cot x \ne \arctan x$

某些资料写 $\sin^{-1}x$ 表示 $\arcsin x$，而负一次幂记号在别的语境又可能表示倒数。为避免歧义，本手册统一使用 $\arcsin,\arccos,\arctan,\operatorname{arccot}$ 表示反三角函数，并把倒数直接写成 $1/\sin x$、$1/\cos x$、$1/\tan x$。
