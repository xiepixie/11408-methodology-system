...
\subsubsection{扩展原型：双曲函数从指数的偶部与奇部长出来}

双曲函数不应另背成一套与指数无关的公式。定义
\[
...
\]
直接给出：$\cosh$ 为偶函数，$\sinh$ 与 $\tanh$ 为奇函数，并且
\[
\boxed{\cosh^2x-\sinh^2x=1}.
\]
从函数对象看，$\sinh:\mathbb R\to\mathbb R$ 严格递增并全局可逆；$\cosh:\mathbb R\to[1,+\infty)$ 因偶性而全局非单射，标准反函数必须先限制到 $[0,+\infty)$；$\tanh:\mathbb R\to(-1,1)$ 严格递增并全局可逆。对应的标准逆函数为
\[
...
\]
\[
\operatorname{artanh}x=\frac12\ln\frac{1+x}{1-x}\quad(\lvert x\rvert<1).
\]
这些闭式都应由“设 $t=e^y>0$ 后解二次方程，再用值域/分支筛根”重建，而不是当成独立公式记忆。完整推导与边界训练由同目录《双曲函数与反双曲函数》承担；导数、有限 Taylor 和根式积分分别转交 Topic03 与 Topic05。
...
\(\sec=1/\cos\)、\(\csc=1/\sin\)、\(\cot=\cos/\sin\)（共同定义域上等于 \(1/\tan\)）是三角函数（倒数/商关系）；\(\arctan,\operatorname{arccot}\) 才是选定分支的映射逆 & 看到上标 \(-1\)、arc、sec/csc 时先判断是在取倒数还是求映射逆 &
把正割/余割或余切误当成反函数 \\
