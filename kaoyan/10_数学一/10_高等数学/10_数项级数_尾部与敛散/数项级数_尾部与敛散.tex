...
\begin{enumerate}
  ...
  \item \kw{Absolute / Cancellation}：先查 $\sum\lvert u_n\rvert$，失败后才研究符号抵消；
...
\node[box,below=of c] (d) {绝对收敛\\$\sum\lvert u_n\rvert$};
...
\[
\left\lvert\sum_{k=n+1}^{m}u_k\right\rvert<\varepsilon.
\]
...
\text{条件收敛}=\text{收敛但 }\sum\lvert u_n\rvert\text{ 发散}.
\]
...
\subsection{基准级数}
\[
\sum q^n\text{ 收敛}\iff \lvert q\rvert<1,\qquad
...
令
\[
\rho=\lim_{n\to\infty}\left\lvert\frac{u_{n+1}}{u_n}\right\rvert,
\qquad
\rho=\limsup_{n\to\infty}\sqrt[n]{\lvert u_n\rvert}.
\]
...
\[
\lvert R_N\rvert=\lvert S-S_N\rvert\le a_{N+1}.
\]
...
\[
\lvert R_N\rvert\le\frac1{\sqrt{N+1}}.
\]
...
\]
这同时给出收敛判别与数值误差。交错级数则优先使用 $\lvert R_N\rvert\le a_{N+1}$，并注意余项符号与首个遗漏项相同。
...
\begin{itemize}
  ...
  \item 若 $\sum\lvert u_n\rvert<\infty$ 且 $\lvert v_n\rvert\le M$，则 $\sum u_nv_n$ 绝对收敛。仅有界而无绝对收敛时，结论不自动成立。
\end{itemize}
\end{methodbox}
...
\begin{enumerate}
  ...
  \item $\sum\lvert u_n\rvert$ 发散被误写成原级数发散；
...
\begin{enumerate}
  ...
  \item 若归零，先问 $\sum\lvert u_n\rvert$ 是否能与基准级数比较；
  \item 比值/根值只在 $\rho\ne1$ 时结案；
