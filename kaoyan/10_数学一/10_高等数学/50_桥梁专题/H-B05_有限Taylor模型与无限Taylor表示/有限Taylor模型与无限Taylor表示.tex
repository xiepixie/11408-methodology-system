...
\begin{center}
\input{assets/src/finite_infinite_taylor_body.tikz}
\end{center}

\begin{tcolorbox}[title=升级资格：两条独立证明路线]
\begin{itemize}
  \item \kw{路线 1（直接余项法）}：若在目标区间或点上直接证明余项 $R_n(x)\to0$（如由 Lagrange 余项界），则级数收敛且和函数自然恒等于 $f(x)$（一步闭合）；
  \item \kw{路线 2（间接展开法）}：若通过已知标准展开式代换、逐项求导或逐项积分得到幂级数，则先求出收敛半径 $R$（内部必然等于 $f(x)$），再在端点单独判定收敛性并用 Abel 定理与端点连续性延拓。
\end{itemize}
\end{tcolorbox}
\subsection*{2.\ 典型路由}
...
\]
固定 $\lvert x\rvert<1$ 时余项趋零，才升级为 $\sum_{k\ge0}x^k$；在 $x=1$ 公式无定义，在 $x=-1$ 余项不趋零。题面从有限近似跳到函数表示时调用这条升级资格。
