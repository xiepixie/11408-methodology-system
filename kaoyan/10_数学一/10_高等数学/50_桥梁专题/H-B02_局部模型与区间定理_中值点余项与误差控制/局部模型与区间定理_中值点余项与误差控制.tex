...
\]
因此若 $\lvert f^{(n+1)}(t)\rvert\le M$，立即得到
\[
...
\begin{center}
\input{assets/src/multiple_mean_values_body.tikz}
\end{center}
...
若 $f$ 在点 $a$ 处二阶可导（即存在有限的 $f''(a)\ne0$），则 $f$ 与 $f'$ 分别享有带 Peano 余项的一阶/二阶展开：
\[
...
\]
更一般地，若 $f$ 在点 $a$ 处 $m$ 阶可导（$m\ge2$），且第一个非零高阶导数是 $f^{(m)}(a)$，即
\[
...
\]
这里的\textbf{接口资格}是点 $a$ 处对应的局部高阶导数存在性；若 $f$ 在附近退化为仿射线性，中值等式对任意 $\theta_h\in(0,1)$ 均成立，说明缺乏非退化高阶导数信息时中值点渐近位置无法被确定。
...
\begin{center}
\input{assets/src/rolle_zero_chain_body.tikz}
\end{center}
...
若已有统一点态误差
\[
\lvert f(x)-P(x)\rvert\le E(x),
\]
则在合法区间上可直接累积为
\[
\left\lvert\int_a^b(f-P)\right\rvert\le\int_a^b E(x)\,dx.
\]
所以“先造余项，再积分”往往比直接攻击复杂积分更稳定。

端点零值还可以与 Rolle/Taylor 共同产生全区间界。例如若 $f(0)=f(1)=0$ 且 $\lvert f''\rvert\le M$，则对每个 $x\in[0,1]$ 可构造二次插值余项得到
\[
\lvert f(x)\rvert\le\frac M2x(1-x),
\qquad
\left\lvert\int_0^1f(x)\,dx\right\rvert\le\frac M{12}.
\]

\begin{center}
\input{assets/src/parabolic_envelope_body.tikz}
\end{center}

这里 $x(1-x)$ 不是要背的模板，而是两个端点零约束生成的二次零点因子。

反向地，局部 Taylor 也能把函数值界与高阶导数界运输成中间导数界。若 $\lvert f\rvert\le M_0$、$\lvert f''\rvert\le M_2$，且 $[x-h,x+h]$ 都在合法区间内，则中央差分与 Taylor 给出一种典型估计
\[
\lvert f'(x)\rvert\le \frac{M_0}{h}+\frac{M_2h}{2}.
\]
...
对 $f(x)=e^x$ 在 $a=0$ 作一阶 Taylor，若 $\lvert x\rvert\le1$，存在 $\xi$ 介于 $0,x$ 之间使
\[
e^x=1+x+\frac{e^\xi}{2}x^2,\qquad
\lvert e^x-1-x\rvert\le\frac e2x^2.
\]
