...
- [二重积分区域、坐标与换序](二重积分区域、坐标与换序.md) —— 从有向积分限恢复真实区域，再选择扫描方向、对称、平移、极坐标与换序；同时覆盖外积分嵌变限积分的升维换序接口，并保留 Jacobian、覆盖与退出检查；
- [三重积分投影切片与坐标](三重积分投影切片与坐标.md) —— 投影/切片分流、柱球坐标参数范围与三重积分结果复核；
- [收缩区域的重积分渐近](收缩区域的重积分渐近.md) —— 收缩区域平均值主阶、Taylor 升阶与区域矩；
- [未知整体积分量回代](未知整体积分量回代.md) —— 把未知整体积分抽成标量并由全区域积分建立自洽方程；
- [混合偏导积分与边界降维](混合偏导积分与边界降维.md) —— 矩形四角差、带权分部积分与非矩形边界退出条件。
...
归档笔记 II-04、II-04.1 及《高数下册进阶》专题 2 已完成当前轮 Source Diff / Owner Diff；逐项路由见 [`2026-08-18_高数下册进阶_Source_Diff.md`](../../../80_evidence/review_log/2026-08-18_高数下册进阶_Source_Diff.md)。补充内容已进入“有向积分限归一化—边界族诱导坐标—参数边界/分段密度—一元嵌套积分升维换序—双重 Riemann 和—乘积域积分不等式”的 Canonical 与训练链，任意平面轴旋转体的微元建模分流到 H-I01。Topic06、Topic07、B02、B07、Topic09 的边界保持不变。正文与同目录训练文档仍需使用者审阅和陌生题验证，当前不标记为“已采用”。
