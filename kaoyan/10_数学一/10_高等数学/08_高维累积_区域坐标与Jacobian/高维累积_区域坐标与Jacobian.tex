...
\begin{warnbox}{累次积分的上下限先区分“方向”与“区域”}
一元定积分允许上限小于下限，此时积分带方向符号；但几何区域的入口/出口必须按较小值到较大值描述。若某段上有 $\varphi_1(x)>\varphi_2(x)$，则
\[
\int_{\varphi_1(x)}^{\varphi_2(x)}f(x,y)\,dy
=-\int_{\varphi_2(x)}^{\varphi_1(x)}f(x,y)\,dy.
\]
因此从累次积分恢复区域前，先把有向积分限归一化；若两条边界在区间内发生交叉，还必须在交点处分段，不能把带符号的积分限直接当作一个无向几何域。
\end{warnbox}
...
\]
原表示先给出区域不等式，水平扫描保持同一三角域，再让困难变量成为外层常参数；正值和 $I<\lvert D\rvert=1/2$ 给出独立数量级检查。不可积结构只是考虑换序的信号；若只把 $dx,dy$ 对调而不重写边界，就已经改变积分对象。
\end{examplebox}

\section{对称性：区域与密度必须共同不变}

若变换 $S$ 保持区域且 $\lvert\det DS\rvert=1$，则可以比较 $f$ 与 $f\circ S$。例如 $D$ 关于 $y$ 轴对称时，关于 $x$ 的奇函数积分为零；若 $D$ 在 $(x,y)\leftrightarrow(y,x)$ 下不变，则
\[
\iint_D f(x,y)\,dA
...
\section{当积分区域本身是决策变量：先按局部贡献的符号选域}

有一类题不是“区域 $D$ 已知，求积分”，而是允许在某个母区域 $U$ 中选择符合条件的子区域 $D$，使
\[
I(D)=\iint_D f(x,y)\,dA
\]
最大或最小。此时 $D$ 本身就是决策变量。若没有面积、形状等额外约束，并且正值集
\[
D_+=\{(x,y)\in U:f(x,y)>0\}
\]
满足题目对区域的可行性要求，那么最大化的生成原则是：\textbf{收入所有正贡献，排除所有负贡献}；$f=0$ 的边界加入与否不改变积分值。于是
\[
\boxed{\sup_D\iint_D f\,dA=\iint_U f_+\,dA,
\qquad f_+=\max\{f,0\},}
\]
并在 $D=D_+$（按题目允许补上零值边界）时取得最大值。最小化同理选择负值区。

关键不是背“取正值区域”，而是逐点比较：若一个可自由增删的小块上 $f>0$，把它加入会增加目标；若 $f<0$，把它删去会增加目标。因此这条结论只在区域允许这样局部调整时成立。若题目额外要求固定面积、指定边界族、连通/单连通而 $D_+$ 本身不满足、或只能从有限个候选区域中选择，就必须先审计可行域；不能把点态符号规则越权成无条件最优性。

这类结构常由 Topic09 的 Green 公式先把闭曲线积分转成 $\iint_D f\,dA$ 后出现；一旦变成“区域选择 + 局部贡献”，优化机制归本册处理，而不再继续在曲线积分表示上比较。
...
\left\lvert\frac{\partial(x,y)}{\partial(u,v)}\right\rvert\,du\,dv.}
\]
...
传统讲义常把两条路线叫作“先一后二”和“先二后一”。这两个名字只是积分执行顺序的简称，真正决定路线的是区域怎样降维：\textbf{投影穿针}先沿一个方向做一维积分（先一后二），\textbf{截面切片}先在固定坐标的二维截面上累积（先二后一）。
...
\[
\boxed{
\frac1{\lvert D_t\rvert}\iint_{D_t}f\,dA\to f(\mathbf x_0),
\qquad
\iint_{D_t}f\,dA=f(\mathbf x_0)\lvert D_t\rvert+o(\lvert D_t\rvert).}
\]
证明只需用连续性把 $\lvert f-f(\mathbf x_0)\rvert$ 一致压到 $\varepsilon$，再乘区域面积。这比追踪一个随 $t$ 变化的中值点更稳健。
...
还要区分“整个区域收缩到一点”和“只在部分方向收缩”。例如
\[
\Omega_t=D_t\times[c,d],\qquad D_t\to\{\mathbf x_0\},
\]
在 $z$ 方向并没有收缩。此时先沿不收缩方向积分，定义有效密度
\[
g(x,y)=\int_c^d f(x,y,z)\,dz,
\]
再研究
\[
\iiint_{\Omega_t}f\,dV=\iint_{D_t}g(x,y)\,dA.
\]
也就是说，\textbf{先把固定纤维压成低维密度，再对真正收缩的横截面做平均值/Taylor 渐近}。若误把这种区域当成三维收缩到一点，会把错误的点值和错误的尺度阶数带进极限。
...
\[
\iint_D f(x,y)\,dx\,dy
=\iint_{D'}f\bigl(T(u,v)\bigr)\lvert\det DT\rvert\,du\,dv,
\]
\[
\frac1{\lvert D_t\rvert}\iint_{D_t}f\,dA\to f(\mathbf x_0)
