...
\section{先固定二阶对象：Hessian 与二次型不是一回事}
对二阶偏导在邻域内足够规则的标量函数 $f:\mathbb R^n\to\mathbb R$，点 $a$ 处的 Hessian 定义为
\[
H_f(a):=\left(\frac{\partial^2 f}{\partial x_i\partial x_j}(a)\right)_{i,j=1}^n.
\]
在常用光滑条件下混合偏导可交换，因此 $H_f(a)$ 是实对称矩阵。它本身是一张坐标矩阵；给定小位移 $h$ 后，对方向真正取值的是二次型
\[
q_a(h):=h^T H_f(a)h.
\]
后文所有“正定、负定、不定”都指这个二次型对所有非零方向的符号结构。
...
记 $H:=H_f(a)$。二阶 Taylor 中真正进入函数增量的是
\[
\frac12 q_a(h)=\frac12 h^THh.
\]
它回答：\textbf{沿当前小方向走出去，二阶主部是向上、向下还是在该方向上暂时看不见变化？}

因此必须区分
\[
\boxed{H\text{ 是矩阵表示},\qquad \frac12 h^THh\text{ 才是二阶 Taylor 主部}.}
\]
...
\item $H$ 不定 $\Rightarrow$ 存在一条方向二阶上升、另一条方向二阶下降，因此必为鞍点；这个结论即使 $H$ 同时奇异也仍然成立；
\item 若 $H$ 半正定或半负定且存在非零零方向，二阶信息一般不足，必须继续看更高阶主部、特殊路径或原函数结构。
\end{itemize}

所以真正的停止条件不是“$\det H=0$”或“$H$ 奇异”这类单一标签，而是：二次型是否已经出现异号方向。已经不定就能判鞍点；只有仍保持单侧符号但存在零方向时，才进入高阶审计。这里“判别失效”只表示\textbf{当前二阶证据不够}，绝不等于“没有极值”。
