...
先固定最小定义。实线性空间上的内积是一个二元函数 $\langle\cdot,\cdot\rangle$，满足对称性、对每个变量的线性以及正定性
\[
...
对于子空间 $W$，还定义它的正交补
\[
W^\perp:=\{r:\langle r,w\rangle=0\text{ 对所有 }w\in W\}.
\]
于是“垂直”与“某个方向集合全部正交”都不再依赖一张图，而成为可计算的代数关系。
...
=\frac{\int_{-1}^{1}x^2\,\mathrm{d}x}{\int_{-1}^{1}1\,\mathrm{d}x}
=\frac13.
\]
于是
\[
...
且
\[
\int_{-1}^{1}\left(x^2-\frac13\right)\,\mathrm{d}x=0.
\]
...
\item 上面的 $\operatorname{proj}_W$ 公式直接针对线性子空间 $W$。仿射子空间要先平移回线性子空间再投影；一般曲线或曲面上的最近点不能直接套线性投影公式。
