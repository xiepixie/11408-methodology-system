...
\]
Jacobian matrix 是 $DT_u$ 在坐标下的表示，维度为 $m\times n$。它记录每个输入方向局部被送到哪里。

只有 $m=n$ 时，才可以继续取
\[
\det J_T(u).
\]
行列式把一张完整的线性映射压缩成一个标量：局部满维小平行体的\textbf{有向体积倍率}。
...
倍。因此普通面积只取大小：
\[
\boxed{dA_x=\lvert\det J_T(u)\rvert\,dA_u.}
\]
三维完全同理：determinant 是局部体积倍率。
...
dA=r\,dr\,d\theta.
\]
这个 $r$ 不是人为补偿：角度变化 $d\theta$ 在半径 $r$ 处对应的切向长度本来就是约 $r\,d\theta$，与径向厚度 $dr$ 相乘自然形成面积 $r\,dr\,d\theta$。

在 $r=0$ 处 determinant 为零，说明角度坐标失去区分能力：所有 $\theta$ 都描述同一个原点。这正是“坐标退化”的几何含义。

\section{局部资格不等于全局覆盖}
\subsection{$\det J_T\neq0$ 只给局部可逆性线索}
它说明局部没有把满维方向压扁；在合适正则条件下可调用逆函数定理得到局部可逆。但它\textbf{不能单独保证整个区域上一一}。
