...
先固定“线性映射”的最低含义：$L:\mathbb R^n\to\mathbb R^m$ 满足
\[
L(\alpha h_1+\beta h_2)=\alpha L(h_1)+\beta L(h_2).
\]
真正的可微要求：存在这样一个线性映射 $L$，使
\[
F(x+h)=F(x)+Lh+r(h),
\qquad
\frac{\lVert r(h)\rVert}{\lVert h\rVert}\to0\quad(h\to0).
\]
此时 $L$ 唯一，记为 $DF_x$。因此“可微”不是“偏导数数量足够多”，而是存在一个同时控制所有小位移的一阶线性主部。
...
\end{cases}
\]
在原点沿每一条固定直线方向的方向导数都存在且为 $0$，但沿曲线 $y=x^2$ 有
\[
f(x,x^2)=\frac12,
\]
所以函数在原点甚至不连续，更不可能可微。这个例子说明：把无穷多条“方向探针”分别算出来，仍可能缺少一个统一控制所有小位移的线性余项。判断可微资格时，最终必须回到统一余项，而不是“导数数量够不够多”。
