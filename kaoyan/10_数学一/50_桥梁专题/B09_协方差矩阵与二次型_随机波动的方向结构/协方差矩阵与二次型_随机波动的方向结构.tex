...
\boxed{\Sigma\succeq0.}
\]
这里 $\Sigma\succeq0$ 就表示“$\Sigma$ 为正半定矩阵”，其定义正是对所有确定向量 $a$ 都有 $a^T\Sigma a\ge0$。

所以“协方差矩阵半正定”不是额外记忆性质，而只是“任意线性组合的方差不能为负”翻译到线代之后的必然结果。

\section{二维相关系数界为什么来自正半定}
若 $\sigma_X^2=\operatorname{Var}(X)>0$、$\sigma_Y^2=\operatorname{Var}(Y)>0$，相关系数定义为
\[
...
这给出一个重要几何读法：若特征向量取单位长度，则
\[
\boxed{\text{单位特征向量 = 二阶波动的主方向，特征值 = 该单位方向上的方差}.}
\]

注意这里只能一般推出这些新坐标两两不相关，不能推出独立；只有联合正态等额外结构才允许把零协方差升级成独立。
...
a^TX=E[a^TX]
\]
几乎必然成立，也就是上述等式以概率 $1$ 成立。换言之，某个非平凡线性组合实际上没有随机波动。

在 $\sigma_X,\sigma_Y>0$ 的前提下，二维中 $\lvert\rho\rvert=1$ 就对应协方差矩阵退化；此时两个中心化变量几乎必然满足非平凡线性关系。这也解释非退化二维正态为什么要求
\[
\lvert\rho\rvert<1.
\]
...
\section{母例：一张协方差矩阵怎样同时给出相关、方向方差与主轴}
设二维随机向量 $X=(X_1,X_2)^T$ 的协方差矩阵为
\[
\Sigma=
\begin{pmatrix}
4&2\\
...
先读最直接的概率信息：
\[
\operatorname{Var}(X_1)=\operatorname{Var}(X_2)=4,
\qquad
...
但真正的桥梁动作不是停在三个元素，而是问任意单位方向 $a$ 的方差。取
\[
e_+=\frac1{\sqrt2}(1,1)^T,
\qquad
e_- =\frac1{\sqrt2}(1,-1)^T.
\]
直接计算
...
因此
\[
\operatorname{Var}(e_+^TX)=e_+^T\Sigma e_+=6,
\qquad
\operatorname{Var}(e_-^TX)=e_-^T\Sigma e_-=2.
\]
同样两个原坐标方差都是 $4$，但沿“同涨同跌”方向的波动强度为 $6$，沿“一增一减”方向只有 $2$。这就是正协方差的方向含义：它不是把每个坐标方差变大，而是把波动重新分配到不同线性组合方向。

若 $X$ 进一步是二维联合正态，则 $e_+^TX$ 与 $e_-^TX$ 不仅协方差为零，而且概率独立；等密度椭圆的两条主轴也正好沿 $e_+,e_-$。于是同一母例一次生成
\[
\boxed{
\text{矩阵元素}
\longrightarrow
