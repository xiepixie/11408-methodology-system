...
PDF 则只属于具有绝对连续分布的情形：若存在非负可积函数 $f$，使对相应可测集合 $A$ 都有
\[
P(X\in A)=\int_A f(x)\,\mathrm{d}x,
\]
则称 $f$ 为 $X$ 的概率密度函数。这里“绝对连续”至少意味着：零长度集合不能承载普通密度意义下的正概率质量；因此单点原子必须另记。
...
因此密度可以大于 $1$，只要总积分仍为 $1$；只要 $X$ 具有普通 PDF，就严格有
\[
P(X=x)=0\qquad\text{对每个 }x.
\]
...
则对 $a<b$，
\[
P(a<X\le b)=F(b)-F(a)=\int_a^b f(t)\,\mathrm{d}t.
\]
...
F'(x)=f(x).
\]
更一般地，若 $F$ 绝对连续，则存在可积函数 $f$ 使
\[
F(x)=\int_{-\infty}^{x}f(t)\,\mathrm{d}t
\]
并且 $F'(x)=f(x)$ 几乎处处。这里“几乎处处”指允许在一个长度为零的例外集合上不成立，并不是“每个点都成立”的同义词。
...
\[
P(a<X\le b)=F(b)-F(a)=\int_a^b e^{-x}\,\mathrm{d}x.
\]
...
是总账；若分布具有密度，则
\[
\boxed{F(x)=\int_{-\infty}^{x}f(t)\,\mathrm{d}t};
\]
在合适正则点再有
\[
\boxed{F'(x)=f(x)}.
