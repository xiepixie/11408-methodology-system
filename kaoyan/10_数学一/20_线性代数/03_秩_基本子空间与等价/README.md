...
- [秩的结构判断与乘积边界](秩的结构判断与乘积边界.md) —— 从独立方向、最大非零子式、$r(A+B)$、转置、参数跳变和 $\operatorname{Im}B\cap\ker A$ 判断 rank，并阻断“rank = 非零特征值个数”等误联想；
- [矩阵等价与基本子空间](矩阵等价与基本子空间.md) —— RREF、row/column/kernel/left-kernel、矩阵等价、初等变换影响、Gram 型矩阵与伴随矩阵 rank 分层。
