...
\section{母模型深化：方程组真正研究的是一个 Fiber}

对固定线性映射 $A:\mathbb R^n\to\mathbb R^m$，定义目标 $b$ 的\kw{纤维（fiber）}
\[
\mathcal F_A(b):=\{x\in\mathbb R^n:Ax=b\}.
\]
线性方程组的全部结构可以压成一个分段模型：
\[
\boxed{
\mathcal F_A(b)=
\begin{cases}
\varnothing, & b\notin\operatorname{Im}A,\\[0.2em]
 x_0+\ker A, & b\in\operatorname{Im}A,\ Ax_0=b.
\end{cases}}
\]
这不是为了增加一个术语，而是把第四章的三类任务放进同一对象：
\begin{enumerate}
  \item \kw{存在性：}fiber 是空还是非空？这由 $b\in\operatorname{Im}A$ 决定；
  \item \kw{内部结构：}若非空，它有多少自由方向？这由 $\ker A$ 决定；
  \item \kw{集合关系：}两个方程组的公共解、包含、同解，就是比较两个 fiber 的交、包含与相等。
\end{enumerate}

因此“解方程”只是把 fiber 写出一份坐标表示；真正稳定的数学对象是 fiber 本身。消元、参数表示、基础解系、特解都是观察或编码这个对象的工具。

\begin{corebox}{两个互补镜头：列空间负责可达，行空间负责约束}
同一个 $Ax=b$ 可以从两端观察：
\[
\boxed{\text{Column/Image view: }b\text{ 能否由 }A\text{ 的列生成？}}
\]
它回答\kw{目标能不能到达}。

另一方面，矩阵的行记录对 $x$ 的线性约束；齐次时
\[
\ker A=\operatorname{Row}(A)^\perp.
\]
所以
\[
\boxed{\text{Row/Kernel view: 哪些方向完全不改变输出？}}
\]
它回答\kw{解集还允许怎样移动}，也直接支撑同解与包含问题。

列空间和行空间不是两套理论：前者检查 fiber 是否存在，后者描述 fiber 的方向与约束。
\end{corebox}

\section{再深一层：Fiber 就是 Kernel 的陪集，也是“同输出”等价类}
...
x-y\in\ker A,
\]
所以“被 $A$ 看成同一个输出”的输入，恰好相差一个 kernel 向量。

因此只要 $b$ 可达，任取 $x_0\in\mathcal F_A(b)$，就有
\[
\boxed{\mathcal F_A(b)=x_0+\ker A.}
\]
也就是说，每个非空 fiber 都是 $\ker A$ 的一个陪集；不同可达输出对应不同陪集。

\begin{corebox}{定义域被 Kernel 分层，Image 只记录“每一层叫什么输出”}
$A$ 把 $\mathbb R^n$ 按照“相差 kernel 向量”的关系分成许多平行层。每一层内部的点全部得到同一个输出，而不同层得到不同输出。
...
n=\dim\ker A+r(A)
\]
可以读成一份信息账本：\kw{输入空间的维数 = fiber 内部看不见的自由度 + 真正能被输出区分的自由度。}
\end{corebox}

这个模型立刻生成三个无需重新消元的结论：
\begin{itemize}
  \item 固定 $A$ 时，所有非空 fiber 都平行，方向空间始终是同一个 $\ker A$；
  \item 若 $b_1\neq b_2$，则 $\mathcal F_A(b_1)$ 与 $\mathcal F_A(b_2)$ 不可能有公共点；
  \item 若 $Au=b_2-b_1$，则
  \[
  \mathcal F_A(b_2)=u+\mathcal F_A(b_1),
  \]
  即右端变化只是在可达层之间搬运整个 fiber。
\end{itemize}
...
\subsection{对偶判据：左零空间给出“无解证书”}

还可以从方程之间的线性依赖来判断相容性。若
\[
y\in\ker A^T,
\]
则
...
Ax=b\text{ 有解}
\iff
y^Tb=0\quad\text{对所有 }y\in\ker A^T.
}
\]

这说明左零空间中的每个 $y$ 都编码了一条“方程之间的线性关系”；右端 $b$ 必须满足完全相同的关系。只要找到一个
\[
y^TA=0,
\qquad
y^Tb\neq0,
...
\begin{examplebox}{为什么消元会出现 $0=c\neq0$ 的矛盾行？}
若某些原方程按系数 $y_1,\dots,y_m$ 线性组合后左端完全抵消，即 $y^TA=0$，但右端组合得到 $y^Tb=c\neq0$，那么组合后的等式就是
\[
0=c,
\]
这正是消元中矛盾行的结构来源。

所以“增广 rank 增加”和“存在左零空间证书”是同一个不可达事实的两种表示。
\end{examplebox}
...
\noindent\begin{tabularx}{\linewidth}{L{3.8cm}L{3.8cm}Y}
\toprule
...
\begin{corebox}{参数题的状态机：fiber 在临界参数处改变类型}
若 $A=A(t)$、$b=b(t)$ 随参数变化，真正需要追踪的不是某个 $x_i$ 的公式，而是
\[
\mathcal F_{A(t)}(b(t))
\]
处于哪一种状态：
\[
\boxed{\varnothing\quad\longleftrightarrow\quad\text{一个点}\quad\longleftrightarrow\quad\text{正维仿射集}.}
\]
状态改变只能发生在 rank / 主元结构发生变化，或右端项在新的 image 中失去相容性的参数点。因此稳定路线是：
\[
\text{找结构跳变点}
\to
\text{判可达性}
...
\noindent\begin{tabularx}{\linewidth}{L{2.2cm}L{2.5cm}Y L{2.4cm}Y}
\toprule
\textbf{A $\neq$ B} & \textbf{为什么容易混} & \textbf{真正判据} & \textbf{题面信号} & \textbf{混淆后果}\\
\midrule
有解 $\neq$ 唯一解 & 可逆时二者同时成立 & 先判 $b$ 可达，再看 kernel 是否为零 & $r(A)$ 与 $r(A\mid b)$ & 跳过自由度判断\\
...
\begin{itemize}
  ...
  \item 看到参数：先定位 $r(A)$ 或 $r(A\mid b)$ 的结构跳变点，再分类；
...
\subsection{独立验证：不要沿原计算路线重算一遍}

得到答案以后按交付类型选择独立检查：
\begin{itemize}
  \item \kw{分类题：}检查结论是否同时满足 $r(A)\le n$、$r(A)\le r(A\mid b)\le r(A)+1$，并确认“唯一/无穷多”只在系统已相容后讨论；
  \item \kw{通解题：}分别验证 $Ax_0=b$、$A\xi_i=0$，并检查 kernel 方向数是否为 $n-r(A)$；
  \item \kw{基础解系：}检查“是解 + 无关 + 数量正确”，不要只验证前两项；
  \item \kw{同解/包含：}齐次时回到 kernel 或 row space；非齐次时回到“共同基点 + 共同方向空间”，避免只比较 rank。
\end{itemize}
...
\]
\[
\boxed{Ax=b\text{ 有解}\iff b\in\operatorname{Im}A\iff r(A)=r(A\mid b).}
\]
\[
