...
\input{assets/src/v07_joint_support_body.tikz}
\diagramcaption{图 4.1\quad 二维联合支撑万能母图（V07）：支撑截面锁定积分上下限}
\end{handbookdiagram}
...
\begin{handbookdiagram}
\input{assets/src/v09_transform_fiber_body.tikz}
\diagramcaption{图 4.2\quad 非单调变换与原像纤维（V09）：常值纤维压缩为原子，多原像分支加总}
\end{handbookdiagram}
...
\begin{handbookdiagram}
\input{assets/src/v08_convolution_slice_body.tikz}
\diagramcaption{图 4.3\quad 卷积截线与三角分布（V08）：等值线截长映射为和的概率密度}
\end{handbookdiagram}
...
\begin{handbookdiagram}
\input{assets/src/v10_jacobian_area_body.tikz}
\diagramcaption{图 4.4\quad Jacobian 面积元微元变换（V10）：局部面积伸缩率与微元概率守恒}
\end{handbookdiagram}
