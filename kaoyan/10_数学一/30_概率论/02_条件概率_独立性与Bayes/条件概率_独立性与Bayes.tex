...
\input{assets/src/v03_conditional_probability_world_shrink_body.tikz}%
}
\end{handbookdiagram}
...
\begin{handbookdiagram}
\resizebox{0.92\linewidth}{!}{%
\input{assets/src/v04_total_prob_and_bayes_bidirectional_tree_body.tikz}%
}
\end{handbookdiagram}
...
本册从这里约定
\[
\boxed{A\indep B\;\Longleftrightarrow\;P(A\cap B)=P(A)P(B)}.
\]
符号 $\indep$ 只表示\textbf{概率独立}，不读作“垂直”；单符号 $\perp$ 留给内积意义下的几何正交。互斥则仍写 $A\cap B=\varnothing$，三种关系不得混用。
若 $P(B)>0$，等价于 $P(A\mid B)=P(A)$；若 $0<P(B)<1$，还等价于 $P(A\mid B^c)=P(A)$。独立是概率关系，不是因果关系。

独立事件的补事件仍独立：$A\indep B$ 蕴含 $A^c\indep B$、$A\indep B^c$、$A^c\indep B^c$。概率为 $0$ 或 $1$ 的事件与任意事件独立，但这属于退化独立，不能用条件概率反向解释，因为某些条件概率没有定义。
...
\[
Y=X+\varepsilon,\qquad \varepsilon\sim N(0,1),\quad \varepsilon\indep X.
\]
于是条件层给出的边缘方差为
\[
E(Y)=E[X]=0,\qquad
...
题设 $A\indep B$、$A\indep C$、$B\cap C=\varnothing$、$P(A)=P(B)=1/2$，且
\[
...
\]
不能把 $A\indep B$ 与 $A\indep C$ 合并成 $B\indep C$；本题正是用互斥而不是独立来处理分母。
\end{examplebox}
