...
\begin{corebox}{本册高频缩写先定义}
累积分布函数（CDF）定义为 $F_X(x):=P(X\le x)$；离散型随机变量的概率质量函数（PMF）定义为 $p_X(x):=P(X=x)$；若存在非负函数 $f_X$ 使
\[
F_X(x)=\int_{-\infty}^{x}f_X(t)\,\mathrm{d}t,
\]
则称 $f_X$ 为概率密度函数（PDF）。CDF 是任意实随机变量都有的通用表示，PMF/PDF 只在相应分布类型下存在；PDF 值不是点概率。
\end{corebox}
...
\input{assets/src/v05_cdf_jump_and_interval_probability_body.tikz}%
}
\end{handbookdiagram}
...
\begin{handbookdiagram}
\resizebox{0.92\linewidth}{!}{%
\input{assets/src/v06_mixture_distribution_decomposition_body.tikz}%
}
\end{handbookdiagram}
...
这里沿用全册记号：$X\indep Y$ 表示随机变量概率独立；单符号 $\perp$ 只用于几何正交。若 $M_X(t)=E(e^{tX})$ 在 $0$ 的邻域存在，则
\[
M_X^{(n)}(0)=E(X^n),\qquad X\indep Y\Longrightarrow M_{X+Y}(t)=M_X(t)M_Y(t).
\]
