...
看到数据以后容易忘记它原本会变化。简单随机样本 $X_1,\dots,X_n$ 是总体分布的独立同分布副本；统计量是样本的函数。正态总体下，样本均值与样本方差拥有特殊的独立性与三大精确抽样分布（$\chi^2, t, F$）。这里先约定：i.i.d. 表示“相互独立且同分布”，只压缩这两个条件。

主线推演逻辑：
\[
\boxed{
\begin{gathered}
\text{总体 } X \xrightarrow{\text{i.i.d. 抽样}} \text{样本 } (X_1,\dots,X_n) \xrightarrow{\text{信息压缩}} \text{统计量 } \bar X, S^2\\
\xrightarrow{\text{正态精确推导}} \chi^2(n-1) \text{ 与 } t(n-1) \text{ 与 } F(m,n)
\end{gathered}
}
\]
\end{corebox}
...
\input{assets/src/v15_sample_orthogonal_decomposition_body.tikz}
\diagramcaption{图 7.1\quad 样本向量正交分解与自由度几何（V15）：均值方向与残差超平面的正交独立性}
\end{handbookdiagram}
...
\begin{warnbox}{数学一范围边界：经验分布保留为拓展，不挤占主干}
本套旧九章资料已明确把“经验分布函数”标为偏数三内容。这里保留它，是因为它能把“统计量仍然随机”“示性变量平均”“大数律”三件事接起来；但数学一复习主干应优先掌握样本均值/方差、$\chi^2,t,F$ 与正态总体抽样分布。除非真题、错题或后续学习需要，不把 Glivenko--Cantelli 等结果作为考场必背项。
\end{warnbox}
...
\]
逐点由强大数律 $F_n(x)\to F(x)$ 几乎必然；整体则有 Glivenko--Cantelli 一致收敛 $\sup_x\lvert F_n(x)-F(x)\rvert\to0$ 几乎必然。重复观测值出现 $m$ 次时 EDF 跳跃高度是 $m/n$，不是固定的 $1/n$。
...
\begin{handbookdiagram}
\input{assets/src/v16_sampling_distributions_lineage_body.tikz}
\diagramcaption{图 7.2\quad 三大抽样分布生成谱系架构（V16）：从标准正态到 $\chi^2, t, F$ 的代数生成关系}
\end{handbookdiagram}
...
\begin{handbookdiagram}
\input{assets/src/v17_quantiles_and_symmetry_body.tikz}
\diagramcaption{图 7.3\quad 抽样分布分位点与对称/倒数反演（V17）：$t$ 负号对称、$F$ 倒数换序与 $\chi^2$ 非对称单尾}
\end{handbookdiagram}

\section{第五层：正态总体的正交分解与精确抽样分布}

若 $X_i\overset{\mathrm{i.i.d.}}\sim N(\mu,\sigma^2)$，则有以下精确结论。这里沿用全册约定：$X\indep Y$ 表示随机变量概率独立；单符号 $\perp$ 只表示几何正交。
\[
\bar X\sim N\left(\mu,\frac{\sigma^2}{n}\right),\qquad
\frac{(n-1)S^2}{\sigma^2}\sim\chi^2(n-1),\qquad \bar X\indep S^2.
\]
...
若 $X\sim t(n)$、$Y\sim F(1,n)$，则 $Y$ 与 $X^2$ 同分布，即 $Y\overset{d}{=}X^2$。当 $c>0$ 且 $P(X>c)=\alpha$ 时，$t$ 分布关于零对称，故
\[
P(Y>c^2)=P(\lvert X\rvert>c)=P(X>c)+P(X<-c)=2\alpha.
\]
...
\begin{corebox}{一句话总结}
\centering
\textbf{样本均值除以未知方差生成 $t$ 分布，样本方差除以总体方差生成 $\chi^2$ 分布，正态样本均值与方差严格独立。}
\end{corebox}
