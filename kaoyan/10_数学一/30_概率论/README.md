...
> 类型：Atlas
> 状态：待人工确认；README 是 Canonical Subject Atlas 候选，八个 Topic 已完成首轮迁移与本轮整体结构审阅，仍需真题/陌生题验证跨册 Owner、术语、参数化与 Rules。考试动作另见[概率统计做题规则](../90_学科做题规则/概率统计.md)。
>
...
本 Atlas 的地图结论不以“公式出现过”作为完成标志。进入 Topic 正文的主干结论，至少要能沿着下面的证据链复原：

$$
\boxed{\text{前提/对象}\;\longrightarrow\;\text{定义或生成机制}\;\longrightarrow\;\text{关键计算/定理映射}\;\longrightarrow\;\text{结论}\;\longrightarrow\;\text{边界}}
$$
...
|---|---|---|
| 条件概率、全概率、Bayes | 条件化的有效世界 → 分割求和 → 观察结果后的反演 | 分母非零、分割完备、先验/似然/后验各自是什么 |
| CDF、PMF、PDF | 事件集合 → 概率表示 → 区间/点概率的计算 | PDF 不是点概率；原子、连续部分和混合分布的边界 |
| 联合、边缘、条件与变换 | 联合支撑 → 对被忽略变量求和/积分或取原像 → 归一化/雅可比 | 支撑集先于积分；非单调变换要分支；条件密度分母合法 |
| 期望、方差、协方差与概率界 | 定义/示性变量 → 线性或平方展开 → 条件分解/不等式 | 存在条件、方差非负、界是上界而非精确值 |
| LLN、CLT 与渐近结论 | 标准化对象 → 波动尺度 → 收敛/极限定理 | 精确分布与近似分布不能混写；独立同分布、方差等假设 |
| 抽样分布与参数推断 | 总体 → 样本统计量 → 抽样分布/枢轴 → 区间或检验 | 统计量身份、自由度、参数域、尾部方向和结论范围 |

概念理解不能停在术语解释。每个一级概念至少要有一组可操作的心智模型：

$$
\boxed{\text{对象}\;\to\;\text{表示}\;\to\;\text{信息操作}\;\to\;\text{保持/丢失什么}\;\to\;\text{题面信号与第一动作}}
$$
