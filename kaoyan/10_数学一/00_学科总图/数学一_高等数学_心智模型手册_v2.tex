...
可导要求误差比 $\lvert h\rvert$ 更高阶：
\[
f(x+h)-f(x)=f'(x)h+o(h)\to0.
\]
所以连续自动成立。但连续只要求函数值不跳，不保证局部变化可以被一个线性斜率统一描述；$\lvert x\rvert$ 在 0 连续却有左右斜率冲突。
...
若
\[
\Delta f=\nabla f(\bm x_0)^T\Delta\bm x+o(\lVert\Delta\bm x\rVert),
\]
...
\[
\boxed{dxdy=\left\lvert\frac{\partial(x,y)}{\partial(u,v)}\right\rvert dudv}.
\]
...
更一般参数面 $\bm r(u,v)$ 有
\[
dS=\lVert\bm r_u\times\bm r_v\rVert dudv.
\]
...
\]
先问哪些 $x$ 使它收敛。中心周围通常形成半径 $R$：$\lvert x-x_0\rvert<R$ 收敛，外部发散，端点必须单独检查。
...
绝对收敛 & $\neq$ & 条件收敛 & 都说明原级数收敛；判据看 $\sum\lvert a_n\rvert$ 是否也收敛，条件收敛依赖正负抵消。 & 错判任意项级数，忽略抵消机制\\
