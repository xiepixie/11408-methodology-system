...
设函数 $f(x)=\begin{cases} x\lvert x\rvert, & x\le 0 \\ x\ln x, & x>0 \end{cases}$，则 $x=0$ 是 $f(x)$ 的
...
设函数 $Q(x,y)=\frac{x}{y^{2}}$，如果对上半平面 $(y>0)$ 内的任意有向光滑封闭曲线 $C$ 都有 $\oint _{C}P(x,y)\,\mathrm{d}x+Q(x,y)\,\mathrm{d}y=0$，那么函数 $P(x,y)$ 可取为
...
设随机变量 $X$ 与 $Y$ 相互独立，且都服从正态分布 $N(\mu ,\sigma ^{2})$，则 $P\{\lvert X-Y\rvert<1\}$
...
设 $\Sigma$ 为曲面 $x^{2}+y^{2}+4z^{2}=4\ (z\ge 0)$ 的上侧，则 $\iint _{\Sigma}\sqrt{4-x^{2}-4z^{2}}\,\mathrm{d}x\,\mathrm{d}y=$
...
设 $a_{n}=\int _{0}^{1}x^{n}\sqrt{1-x^{2}}\,\mathrm{d}x\ (n=0,1,2,\cdots)$。
...
设向量组 $\alpha _{1}=(1,2,1)^{T}$，$\alpha _{2}=(1,3,2)^{T}$，$\alpha _{3}=(1,a,3)^{T}$ 为 $\mathbb R^{3}$ 的一个基，$\beta =(1,1,1)^{T}$ 在这个基下的坐标为 $(b,c,1)^{T}$。

（Ⅰ）求 $a,b,c$；

（Ⅱ）证明 $\alpha _{2},\alpha _{3},\beta$ 为 $\mathbb R^{3}$ 的一个基，并求 $\alpha _{2},\alpha _{3},\beta$ 到 $\alpha _{1},\alpha _{2},\alpha _{3}$ 的过渡矩阵。
...
$$
f(x;\sigma ^{2})=\begin{cases} \frac{A}{\sigma}\mathrm{e}^{-\frac{(x-\mu )^{2}}{2\sigma ^{2}}}, & x\ge \mu , \\ 0, & x<\mu . \end{cases}
$$
