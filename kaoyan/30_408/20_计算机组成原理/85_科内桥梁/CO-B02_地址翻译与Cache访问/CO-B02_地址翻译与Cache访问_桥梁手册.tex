...
\boxed{
\begin{aligned}
\text{VA Access Request}
&\to\text{Translation Result Packet}\\
&\to\text{Cache Address / Attribute Packet}\to\text{Hit/Miss Path}
\end{aligned}}
\]
...
\section{三类快路径失败必须保持分离}
\begin{center}\small
\begin{tabularx}{\linewidth}{L{2.2cm}YYY}\toprule
事件 & 缺失对象 & 处理路径 & 重试点\\\midrule
TLB miss & 翻译缓存副本 & page walk / software refill（按架构） & 重新形成 PA\\
Translation fault & non-present / residency / permission 等翻译条件 & 精确异常交给 OS & 修复后 retry 或拒绝/终止\\
Cache miss & 数据块副本 & victim / writeback / fill & refill 后 retry/complete\\
\bottomrule
\end{tabularx}
\end{center}
