...
\[
\boxed{\begin{aligned}
\text{Program Intent}&\to\text{Architectural State}\\
&\to\text{Encoding / Address Semantics}\to\text{Next Architectural State}
\end{aligned}}
\]
...
\section{从程序意图到可执行映像：先按中间产物拆阶段}
“高级语言能运行”不是编译器一步把文字变成 CPU 动作。408 的阶段化模型应先按\kw{输入/输出产物}拆开：
\[
\begin{aligned}
\text{Source}&\xrightarrow{\text{preprocess}}\text{Translation Unit}
\xrightarrow{\text{compiler}}\text{Assembly}\\
&\xrightarrow{\text{assembler}}\text{Relocatable Object}
\xrightarrow{\text{linker}}\text{Executable / Shared Object}.
\end{aligned}
\]
程序真正开始运行时才进入下一条 OS 路径：
\[
\text{Executable}
\xrightarrow{\text{loader / exec}}
\text{Process Image + Entry State}
\xrightarrow{\text{CPU}}
\text{Execution}.
\]

预处理处理宏、条件编译和头文件展开；编译器把高级语言语义映射为 ISA 可表达的指令与数据布局；汇编器把助记符、伪指令和标签变成机器码，并产生符号表与重定位记录；链接器解析跨目标文件的符号引用、安排代码/数据位置并据此修补或保留重定位；装入器则消费已经形成的可执行映像，建立进程地址空间、初始栈和入口状态。命令行中的 \code{gcc} 常作为 driver 一次串起多步，但工具被一个命令包装，不表示这些语义阶段合并了。

\begin{methodbox}{题目问“生成可执行文件”时在哪里停止？}
若题目问“高级语言源程序怎样转换成可执行目标文件”，链条停止在\kw{链接}：
\[
\boxed{\text{预处理}\to\text{编译}\to\text{汇编}\to\text{链接}}.
\]
Loader 消费 executable 并开始建立运行环境，它不属于“把源程序生成可执行文件”的最后一步。
\end{methodbox}

这条链也解释了四个边界：编译器选择不等于 ISA 必需序列；可重定位目标文件还允许存在未决外部引用；链接地址不等于运行时物理地址；装入完成不等于第一条指令已经提交。题目给出目标文件、符号或重定位项时，先判断它处在哪一个阶段，再决定谁有足够信息修改它。

\begin{center}\small
\begin{tabularx}{\linewidth}{L{2.0cm}YY}
\toprule
阶段 & 主要输入/输出 & 不能直接推出的事实\\\midrule
预处理 & 源文件 $\to$ 展开后的翻译单元 & 已经生成机器指令\\
编译 & 高级语言语义 $\to$ 汇编/低级表示 & 唯一指令序列或固定寄存器分配\\
汇编 & 汇编源码 $\to$ 可重定位目标文件 & 所有符号和地址已经最终确定\\
链接 & 多个目标文件/库 $\to$ 可执行或共享对象 & 已经位于某个进程的物理内存\\
装入 & 可执行映像 $\to$ 进程映像与入口状态 & 所有页面已驻留或程序已经执行\\
\bottomrule
...
\begin{center}
\input{assets/src/effective_address_layers_body.tikz}
\end{center}
...
\subsection{类型先于机器码：显式转换的位置改变状态转移}
把 C 表达式映射到机器级状态时，必须先保留源语言规定的\textbf{子表达式类型与求值顺序}，再调用 CO-01 的有限格式转换。典型对照是
\[
\texttt{(double)(x+y)}\qquad\text{vs}\qquad\texttt{(double)x+(double)y}.
\]
前者先完成 $x+y$ 的源类型运算，再把结果转换为 double；后者先把两个操作数转换成 double，再执行浮点加法。因此目的类型不能反向修改转换之前已经发生的运算宽度或溢出边界。若源语言把某个中间状态定义为未定义、实现相关或需要异常处理，题设必须给出相应口径；不能因为底层补码硬件存在一个回绕位串，就自动把它升级成源程序的可移植语义。

CO-01 输出的是“某个源值经过扩展、截断或浮点舍入后得到什么目标值/位模式”；CO-02 消费这份结果，把它放回表达式、寄存器和访存状态中。这个接口也解释了为什么 `int32 -> double -> int32` 可以在可表示范围内无损，而 `float -> int -> float` 往往因为第一次向整数转换已经截断小数而无法恢复。

\subsection{控制流}
条件结构通常变为比较、条件选择下一 PC 和无条件跳转形成回边。分支目标可以是 PC 相对、寄存器间接或 ISA 规定的其他形式；比较是否产生显式标志、是否与分支合并，由 ISA 决定。

\subsection{比较结果怎样成为条件转移的输入}
CO-01 负责产生有限宽度减法结果与 $ZF/CF/SF/OF$ 等候选状态；本册只解释 ISA 怎样消费这些证据。对 408 常见的“先做 $A-B$、再查条件码”模型：
\begin{itemize}
  \item $A=B$ 时，结果为零，所以 $ZF=1$；
  \item 有符号比较不能只看 $SF$。overflow 会让机器结果的表面符号与数学符号相反，因此常见条件码用 $SF\oplus OF$ 校正：$A<B$ 对应 $SF\oplus OF=1$，$A>B$ 还需 $ZF=0$ 且 $SF\oplus OF=0$；
  \item 无符号比较看 zero 与 carry/borrow，但必须先锁定题设对 $CF$ 的口径。若题目规定减法 $CF=1$ 表示借位，则 $A<B$ 对应 $CF=1$，$A>B$ 对应 $ZF=0\land CF=0$。
\end{itemize}
这不是“所有 ISA 都有同一组 flags”的宣言：有的 ISA 用 no-borrow carry，有的比较指令直接写普通寄存器，有的分支直接比较两个寄存器。稳定方法是先写\textbf{比较产生了什么逻辑谓词}，再按题设把谓词映射到 condition code / branch condition。
...
\begin{center}
\input{assets/src/abi_call_frame_body.tikz}
\end{center}
...
\begin{enumerate}
  ...
  \item 若是 compare / 条件转移，先写实际比较谓词和 $A-B$；signed 用 $SF\oplus OF$ 校正结果符号，unsigned 先确认 carry/borrow 口径，再读取对应条件；
...
\[
\begin{aligned}
\text{Instruction Format}&\to\text{Operand Interpretation}\to\text{EA / Target Formation}\\
&\to\text{Read/Write Set}\to\text{Next Architectural State}.
\end{aligned}
\]
...
\[
\text{Assembly}
\xrightarrow{assembler}\text{Relocatable Object}
\xrightarrow{linker}\text{Executable / Shared Object}
\xrightarrow{loader}\text{Process Image}.
\]
可重定位目标文件已经含机器指令和静态数据，但还可能保留未定义外部符号以及“将来必须修补哪里”的 relocation records。链接器拥有多个模块的整体布局后，才能完成 symbol resolution 与相应 relocation；Loader 再把可执行表示转成运行时地址空间。\kw{链接阶段的重定位}与\kw{运行期 VA $\to$ PA 地址翻译}不是同一个动作：前者修补文件/映像中的地址相关引用，后者由 OS+MMU 在程序运行时解释虚拟地址。完整链接/装入机制转交 OS-00，运行期地址翻译转交 OS-04 / CO-07。
