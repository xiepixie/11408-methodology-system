...
- **Q334**：虽然与分页有关，但考的是页/页框放置、碎片和分段/分页保护边界，Canonical Owner 在 OS 虚拟内存，不应因为题号邻近就把它塞进 CO-07。
- **Q335**：缺页由正在执行的访存同步触发，硬件报告 fault，OS 修复并 retry；它不是“TLB 没命中以后去慢表”这一普通快路径，也不是外部设备异步中断。
- **Q336**：TLB 之所以能成为快表，核心是小容量高速相联结构可按 VPN 身份快速匹配；不是因为采用“更聪明的搜索算法”，更不能用“TLB 命中率高于页表”解释。页表是权威映射结构，TLB 是其中近期可用 translation 的高速副本，两者不在同一个命中率分母上比较。
- **Q333（05 批）**：再次验证 `TLB miss / translation fault / Cache miss` 三种失败对象必须分流；只有翻译成功形成合法访问身份后，普通物理 Cache hit/miss 才有本次访问的语义。

这批攻击没有要求重建分页母模型；真正被加强的是 **Owner 路由、翻译副本 vs 权威映射、以及 fast-path miss vs fault** 三个边界。

正式题面与解析：

- [05｜Cache、局部性与存储层次](../../../archives/408/题库/计算机组成原理/05_Cache局部性与存储层次_选择题.md)
- [06｜虚拟存储、标志寄存器与指令系统](../../../archives/408/题库/计算机组成原理/06_虚拟存储标志寄存器与指令系统_选择题.md)
