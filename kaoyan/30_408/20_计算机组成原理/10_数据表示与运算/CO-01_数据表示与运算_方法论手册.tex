...
本册覆盖定点与浮点表示、ALU 加法器、乘除迭代、校验码、类型转换、大小端和对齐。并行阵列只是“用更多硬件换更短时间”的一种实现；Booth 或不恢复除法的寄存器安排和拍数必须以题设为准。
...
\subsection{C 类型转换、大小端与对齐：位串搬运也有语义边界}

C 类型转换题先区分“位模式是否改变”和“解释是否改变”：
\begin{itemize}
  \item 同宽 signed/unsigned 转换常保持位模式，只改变解释；
  \item 变宽时，unsigned 补零，signed 补符号位；
  \item 变窄时保留低位，可能丢失高位信息；
  \item 整型转浮点或反向转换还要经过不同格式的精度/范围映射。
\end{itemize}
最稳的检查是“截后再扩”：把窄化结果按目标类型重新扩回，若不能恢复原值，则转换有信息损失。高位重复并不自动等于“安全”，必须相对于目标解释判断。

Endian 只规定多字节对象的字节在递增地址上的排列，不改变寄存器中对象的数学值。解题时先画地址—字节表，再读取指定宽度；字节序和位序不能互换。对齐是起始地址相对于对象大小的约束，常见条件为 $a\equiv0\pmod{k}$。未对齐访问可能被拆成多次事务、被硬件支持但变慢，或触发异常，具体由 ISA/题设决定；不能把某一平台的实现惩罚升级成统一语义。
