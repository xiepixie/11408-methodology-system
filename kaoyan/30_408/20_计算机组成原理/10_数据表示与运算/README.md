...
本 Topic 拥有位权、位宽、unsigned/signed、原码与补码、扩展/截断/移位、ALU 标志、定点乘除基本算法、溢出、除零检测、一般浮点格式的基数/阶码/尾数/规格化，以及 IEEE 754 编码、运算链与舍入。
...
- [移位、Booth 乘法与迭代除法](移位、Booth乘法与迭代除法.md)：锁定位宽/解释/寄存器布局后，训练逻辑/算术移位、Radix-2 Booth 逐拍表、恢复/不恢复除法与积/商余回算。
- [一般浮点格式、规格化与范围](一般浮点格式、规格化与范围.md)：训练非 IEEE 教材浮点格式的 `base / exponent encoding / mantissa encoding / normalization` 恢复，尤其处理基数 4、补码尾数规格化与表示范围端点生成。
...
- [类型转换与混合表达式](类型转换与混合表达式.md)：训练 `signed/unsigned`、整数位宽变化、`int/float/double` 往返转换与 cast 位置；核心判断是“当前这一步在哪种类型中计算、第一步转换是否丢信息”。
