...
\[
\boxed{\begin{aligned}
\text{ISA State Delta}&\to\text{Required Values}\to\text{Producer--Consumer Graph}\to\text{Datapath}\\
&\to\text{Resource Schedule}\to\text{Control Word}\to\text{Clock Boundary}\to\text{Precise Commit}
\end{aligned}}
\]
...
\[
\boxed{\begin{aligned}
\text{Datapath: }&\text{what values can move and transform}\\
\text{Control: }&\text{which legal movement happens now}
\end{aligned}}
\]
...
\begin{center}
\input{assets/src/single_bus_microops_body.tikz}
\end{center}
...
\begin{center}
\input{assets/src/microprogram_control_loop_body.tikz}
\end{center}
...
\[
\begin{aligned}
\text{Architectural State Delta}&\to\text{Value / Resource Dependency}\to\text{Micro-operation Schedule}\\
&\to\text{Control Word / Next State}\to\text{Clocked Commit}.
\end{aligned}
\]
...
\end{itemize}
操作控制字段可以采用水平型直接编码、垂直型字段编码或混合编码。直接编码并行度高但微指令长，字段编码节省控制存储器但同一字段内不能同时打开互斥动作。字段直接编码若把一个含 $m$ 个互斥微命令的集合放进同一字段，通常还要保留“本字段不发微命令”的编码，因此该字段最少需要
\[
\left\lceil\log_2(m+1)\right\rceil
\]
位；只有题设明确每拍必选一个动作时才可省掉空操作状态。多个互斥类的字段宽度分别计算后再相加。控制存储器保存的是微指令，但仍由微地址选择，不因为名称中有“控制”就变成按内容匹配的 CAM。微地址可以顺序加一、条件分支、按 opcode 映射或跳回取指入口；“微程序控制就是顺序执行微指令”是不完整模型。
