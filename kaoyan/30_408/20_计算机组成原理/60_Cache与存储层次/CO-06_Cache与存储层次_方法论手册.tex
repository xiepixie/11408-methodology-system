...
\subsection{地址位预算是不变量：几何参数变化时先看谁把位让给谁}
对字节编址、2 的幂几何，地址宽度固定为 $A$ 时始终满足
\[
\boxed{A=tag+index+offset}.
\]
...
可以直接生成常见微扰：
\begin{itemize}
  \item 固定 Cache 数据容量 $C$ 与相联度 $E$，块大小 $B$ 加倍：$offset+1$，组数减半所以 $index-1$，二者抵消，因此 $tag$ 不变；
  \item 固定块大小和总 line 数，相联度 $E$ 加倍：组数减半，所以 $index-1$；$offset$ 不变，因此 $tag+1$；
  \item 固定地址宽度与组数，块大小加倍：$offset+1$ 而 $index$ 不变，因此 $tag-1$。
\end{itemize}
所以“相联度提高”“块变大”都不能脱离\textbf{哪些总量保持不变}直接推出 Tag 变化。
...
\begin{center}
\resizebox{0.96\linewidth}{!}{\input{30_408/20_计算机组成原理/60_Cache与存储层次/assets/src/cache_mapping_body.tikz}}
\end{center}
...
\begin{center}
\input{assets/src/cache_write_policy_body.tikz}
\end{center}
