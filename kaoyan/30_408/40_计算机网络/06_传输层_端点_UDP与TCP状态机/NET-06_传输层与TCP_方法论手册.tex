...
\begin{center}
\resizebox{0.96\linewidth}{!}{\input{30_408/40_计算机网络/06_传输层_端点_UDP与TCP状态机/assets/src/tcp_lifecycle_body.tikz}}
\end{center}
图把正常主动打开与主动关闭放在同一双端点时间线上：第三次握手完成 server 对 client 已收到其 ISN 的知识闭环；FIN 只关闭一个方向；TIME-WAIT 由主动关闭事件决定，而不是固定属于 client。
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.96\linewidth}{!}{\input{assets/src/tcp_flow_control_rwnd_body.tikz}}
  \caption{TCP 流量控制：rwnd 通告、零窗口死锁与持续计时器探测}
  \label{fig:tcp-rwnd-flow-control}
\end{figure}
