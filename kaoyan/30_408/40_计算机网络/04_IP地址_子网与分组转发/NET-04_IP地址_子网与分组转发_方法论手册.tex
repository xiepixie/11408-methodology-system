...
prefix/CIDR & 地址前若干位相同的一组地址；\code{/p} 表示前缀长度 $p$ & 不是“某台主机的完整地址” \\
...
本册所有“转发”都指当前设备已经有了可用的 FIB 动作；路由协议如何产生候选路线
属于控制平面知识，但这里仍给出 RIB/FIB 的定义，所以读者无需打开 NET-05 才能理解
后文的分组处理。
...
\section{字段级核验：每个长度、地址与校验和都绑定一个作用域}

\subsection{分类编址是历史解释层，CIDR 才是当前匹配层}

\begin{center}\small
\begin{tabularx}{\linewidth}{L{1.4cm}L{2.2cm}L{2.1cm}Y}
\toprule
类别 & 首位模式/首字节 & 历史默认前缀 & 当前应怎样使用\\
\midrule
A & \code{0}，通常 1--126 & \code{/8} & 只用于识别旧分类题；0 与 127 有特殊语义\\
B & \code{10}，128--191 & \code{/16} & 不可替代题设给出的 CIDR prefix\\
C & \code{110}，192--223 & \code{/24} & 同上\\
D & \code{1110}，224--239 & multicast & 组地址，不是普通 host unicast\\
E & \code{1111}，240--255 & reserved & 不能当普通可分配地址\\
\bottomrule
\end{tabularx}
\end{center}

分类编址的问题不是“记忆麻烦”，而是固定 A/B/C 块无法贴合需求，造成地址浪费和路由状态膨胀。CIDR 用任意 prefix length 同时支持 VLSM、聚合与 LPM。主机号全 0 表示该前缀本身、全 1 在传统 IPv4 子网模型中表示定向广播；\code{0.0.0.0} 可作未配置主机的 bootstrap source，\code{255.255.255.255} 是受限广播目的，\code{127.0.0.0/8} 是 loopback。题目必须先辨语境，不能把这些特殊值当普通接口地址。

\subsection{IPv4 三个长度字段为何使用三种单位}

\begin{center}\small
\begin{tabularx}{\linewidth}{L{2.5cm}L{1.5cm}L{1.8cm}Y}
\toprule
字段 & 位宽 & 单位/范围 & 设计与计算出口\\
\midrule
...
\subsection{IPv4 Header Checksum：全 1 与全 0 是同一次验证的两步}

发送端把 checksum 字段置零，将首部切成 16-bit words，用反码加法求和；最高位进位回卷到最低位，再对和取反写入字段。接收端把包含 checksum 的首部同样相加：
\[
\boxed{\text{sum before complement}=0xFFFF}
\qquad\Longleftrightarrow\qquad
\boxed{\text{result after complement}=0x0000}.
\]
...
\subsection{IPv6 的“无需 DHCP”必须校正为“不强制依赖 DHCP”}
...
\section{控制报文与设备动作：把 补充笔记中的数值细节接回逐跳链}

\subsection{ICMP：何时报告、抄回什么、何时保持沉默}
...
\subsection{DHCP：DORA 之外还有端口、租约时钟与 relay}

DHCPv4 client/server 使用 UDP 68/67。未配置 client 可从 \code{0.0.0.0} 向 \code{255.255.255.255} 发送 DISCOVER；REQUEST 继续广播还让未被选中的 server 收回 offer。租期 $T$ 的教材型时钟为
\[
T_1=0.5T,\qquad T_2=0.875T.
\]
...
\subsection{NAT/NAPT：作用域、容量与两层 checksum}

RFC 1918 private blocks 为
\[
\code{10.0.0.0/8},\qquad
\code{172.16.0.0/12},\qquad
\code{192.168.0.0/16}.
\]
...
\subsection{IPv4 Multicast 到 Ethernet MAC：32:1 映射必须二次过滤}

IPv4 multicast 的低 23 bit 复制到 Ethernet multicast 前缀 \code{01-00-5E} 后；MAC 的前 25 bit 固定，其中第 4 byte 最高位为 0。D 类地址可变 group 部分有 28 bit，因此有 5 bit 被丢弃：
\[
2^5=32\quad\Longrightarrow\quad
\text{32 个 IPv4 multicast groups 可映射到同一 MAC}.
\]
网卡按 MAC 过滤只能得到一个粗集合，host 的 IP 层还必须按 destination IP 再过滤。这个映射解决最后一跳硬件交付，不生成 IGMP membership，也不生成路由器间 multicast tree。

\subsection{Router fabric 与队列：逐包快路径的物理落点}

输入端口依次完成 PHY 接收、链路解封装与 FIB lookup；控制报文可送 routing processor，数据 packet 进入 switching fabric。为达到 line rate，FIB 可在各输入端口保留受控的 shadow copy，由控制面统一更新。三种经典 fabric 的瓶颈不同：
...
结构 & 主路径 & 瓶颈/并发边界\\
\midrule
Memory & input $\to$ memory $\to$ output & 每 packet 至少一次写、一次读，受 memory bandwidth 限制\\
Shared bus & input 在总线上标记 output & 同一时刻总线只承载一个 packet，受 bus rate 限制\\
Crossbar/\newline interconnect & 独立 input-output 交叉点 & 不同输出可并行；争用同一 output 时仍需仲裁/排队\\
\bottomrule
\end{tabularx}
\end{center}
...
\section{来源覆盖附录：NET-04 逐 Source 核销}

\begin{center}\small
\begin{tabularx}{\linewidth}{L{4.1cm}L{3.0cm}Y}
\toprule
Source & 本册承接节点 & 核销结论\\
\midrule
\code{network/}\newline\code{network-layer-functions} & 转发/互联接口 & 数据面转发、异构互联、FIB/LPM 由本册承接；route generation 路由 NET-05，拥塞反馈路由 NET-07\\
\code{network/}\newline\code{ip-addressing} & prefix/same-subnet/LPM & 分类边界、特殊地址、subnet/VLSM、CIDR、aggregation overreach、直接/间接交付已展开\\
\code{network/}\newline\code{ip-datagram} & IPv4 header & 字段位宽、三种长度单位、Protocol/TTL、Options 与反码 checksum 已展开\\
\code{network/}\newline\code{ip-fragment} & MTU/fragmentation & payload 对齐、offset、DF/MF、二次分片状态、目的重组与 IPv6 Owner 变化已展开\\
\code{network/}\newline\code{arp} & next-hop identity & 同/异网段对象、request 广播/reply 单播、四类调用、cache aging 与 spoofing 边界已展开\\
\code{network/}\newline\code{icmp} & 网络层反馈 & 报文类型、原 header+8 B、不反馈四类、ping/traceroute 与非可靠性已展开\\
\code{network/}\newline\code{nat} & tuple rewrite & private blocks、Basic NAT/NAPT、正反向表、双 checksum、无端口协议/状态/穿透边界已展开\\
\code{network/}\newline\code{ipv6} & IPv6 address/header & 8 字段、扩展首部、压缩可逆性、地址类型/作用域、SLAAC/DHCPv6 校正与过渡已展开\\
\code{network/}\newline\code{multicast} & group/membership & D 类、32:1 MAC 映射、IP 二次过滤、IGMP membership 与 tree state 分离已展开\\
\code{network/}\newline\code{mobile-ip} & identity/locator & MN/HA/FA/CN、两类 CoA、发现/登记/隧道、三角路由、直接路由与移动更新已展开\\
\code{network/}\newline\code{router} & router data plane & input shadow FIB、line rate、三种 fabric、input/output queues、HOL 与 RIB/FIB 边界已展开\\
\code{application/}\newline\code{ftp-dhcp} & DHCP 部分 & UDP 67/68、DORA、广播层次、lease $T_1/T_2$、NAK/expire 与 relay 已展开；FTP 路由 NET-08\\
\bottomrule
\end{tabularx}
\end{center}
