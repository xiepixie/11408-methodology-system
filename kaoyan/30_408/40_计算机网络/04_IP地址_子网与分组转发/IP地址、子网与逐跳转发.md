...
![CIDR 二进制子网划分、地址池分配与 DHCP 四步配置](./assets/CIDR定长子网划分与DHCP动态配置.svg)

图中顶部展开 `166.1.0.0/16` 的二进制前缀结构：网络前缀 16 位固定，子网位扩展划分出 4 个定长子网（`/18`），剩余 14 位主机位提供地址池；下方展示校园网拓扑中各路由器接口的精确子网配置与主机通过 DHCP（Discover $\to$ Offer $\to$ Request $\to$ ACK）广播获取 IP 租约的全过程。
...
![多级路由最长前缀匹配 (LPM) 与 NAT/NAPT 双向端口映射](./assets/NAT网络地址转换与多级路由最长前缀匹配.svg)

图中左侧展示家庭私网通过 NAT 路由器将内部地址 `(192.168.3.2:3345)` 转换为全局公网 `(218.75.26.10:50001)` 的双向端口映射表；右侧展示数据包进入 Internet 后，多级路由器如何基于 FIB 路由表的前缀长度执行最长前缀匹配（LPM），优先匹配最精确的子网路由而非默认路由。
