...
\begin{warn}[Legacy Source｜禁止作为当前 Owner]
本文件是旧版聚合 Source，不再拥有任何稳定定义。当前唯一 Owner：Priority Inversion / inheritance / ceiling → OS-03；MMIO / Port I/O → CO-08 与 X-B01/X-B03（OS-05 仅 Use 接口）；Process × File → OS-B03；VM × File/Page Cache → OS-B04。若本文件与当前 Topic / Bridge 冲突，以当前 Canonical Owner 为准。
\end{warn}
...
&\xrightarrow{\text{File Descriptor } fd} \text{Open File Description (OFD / System-wide Table)} \\
&\xrightarrow{\text{Inode Index}} \text{Inode Object (FileSystem)}
\end{aligned}
}
\]
...
&\xrightarrow{\text{Page Fault}} \text{Page Cache (RAM Physical Frame)} \\
&\xrightarrow{\text{Dirty Writeback}} \text{Inode Block Mapping (Disk)}
\end{aligned}
}
\]
