...
\begin{method}[术语预备：名字、对象与持久数据]
\begin{itemize}[leftmargin=1.5em]
  \item \kw{文件对象}：可按字节读写并具有生命周期的逻辑对象；\kw{文件名}只是目录中指向它的一条名称记录，不是对象身份本身。
  \item \kw{元数据 (metadata)}：描述对象类型、大小、权限、时间、所有者和数据位置的信息；它与文件内容分开管理。
  \item \kw{目录项 (directory entry)}：目录文件中的“名字 $	o$ 对象标识”记录；\kw{dentry} 是 Linux VFS 在内存中缓存路径组件的对象，不能直接当成磁盘目录项。
  \item \kw{FCB/inode}：FCB 是教材中的文件控制块；inode 是 Unix 类系统保存文件身份、元数据和数据定位信息的结构。两者都不是文件名本身。
  \item \kw{文件描述符 (fd)}：进程文件描述符表中的小整数索引；\kw{OFD} 是它指向的打开文件对象，保存当前偏移和打开方式；\code{fd}、OFD、inode 是三层不同对象。
  \item \kw{ACL}：Access Control List，按对象记录哪些主体拥有何种权限；\kw{Capability}：按主体持有可访问对象及权限的凭证集合。
  \item \kw{VFS}：Virtual File System，向应用提供统一的文件 API，并把路径操作转发给具体文件系统实现；\kw{mount} 把一个文件系统接入现有目录命名空间。
  \item \kw{页缓存 (Page Cache)}：以内存页为单位缓存文件内容，供普通读写和 \code{mmap} 共享；它不是 CPU 的硬件 Cache。
  \item \kw{日志/预写日志 (Journaling/WAL)}：先把元数据变更意图按规定顺序写入日志，崩溃恢复时据此重放或撤销，保证文件系统结构不落入非法中间状态。
\end{itemize}
后文遇到路径、\code{open}、\code{fork}、硬链接或 \code{mmap} 时，都沿着“名字 $	o$ 对象 $	o$ 打开引用 $	o$ 数据位置 $	o$ 持久化约束”这条链判断。
\end{method}
...
\[
\begin{aligned}
\text{State (Bitmap/Inode/Data Blocks)} &\to \text{Crash/Power Fault (突发断电)} \\
...
\subsection{文件逻辑结构、访问方式与物理分配的三层辨析}
...
\subsection{文件保护模型：Access Matrix 与 ACL / Capability 投影}

文件的保护与安全机制建立了“谁可以对哪个对象做什么动作”的边界。

\paragraph{通用保护模型：访问矩阵 (Access Matrix)}
任何系统保护抽象都可表示为一个二维访问矩阵 $\text{AccessMatrix}$：
\[
\boxed{\text{AccessMatrix}[\text{Subject / Domain}, \text{Object}] = \text{Rights}}
\]
其中行代表主体/保护域 (Subject/Domain，如用户、进程、角色)，列代表受保护对象 (Object，如文件、目录、设备)，矩阵元素表示具体的许可权限（如 \code{r}, \code{w}, \code{x}）。
...
\midrule
按列 (Object) 存储 $\to$ \kw{ACL} & 将列上的主体及其权限挂在**对象 (File)** 下；形式为：$\code{Object} \to \{(\code{Subject}_i, \code{Rights}_i)\}$ & **访问控制列表 (Access Control List)**；如 Linux 文件 \code{rwxr-xr--}、posix ACL。删除文件时易一并回收。\\
按行 (Subject) 存储 $\to$ \kw{Capability} & 将行上的对象及其权限挂在**主体 (Domain)** 手里；形式为：$\code{Subject} \to \{(\code{Object}_j, \code{Rights}_j)\}$ & **能力列表 (Capability List)**；进程持有一组令牌/凭证。传递与撤销权限粒度更细，但查询“某文件被谁有权访问”开销高。\\
\bottomrule
\end{tabularx}
\end{center}

\begin{corebox}[ACL 与 Capability 都来自访问矩阵]
ACL 与 Capability 根本不是两个孤立无因的名词，它们是**同一个 Access Matrix 的两种存储切割与二维矩阵投影**！
\end{corebox}

\section{第 2 章：目录、路径与链接——从名字到对象}

\subsection{目录：把文件名映射到文件对象}
...
\subsection{Mount：路径经过挂载点时切换文件系统实例}
...
\begin{center}
\begin{tikzpicture}[x=3.30cm,y=1.55cm]
  % independent
  \node[font=\small\bfseries] at (0.45,0.65) {Independent open};
  \node[fsnode] (a1) at (0,0) {fd=3\\[-1pt]\scriptsize process A};
  \node[fsnodep] (a2) at (0,-1) {OFD \#1\\[-1pt]\scriptsize offset=100};
  \node[fsnodeg] (ai) at (0.45,-2) {inode X};
  \draw[refedge] (a1) -- (a2); \draw[refedge] (a2) -- (ai);
  \node[fsnode] (b1) at (0.9,0) {fd=7\\[-1pt]\scriptsize process B};
  \node[fsnodep] (b2) at (0.9,-1) {OFD \#2\\[-1pt]\scriptsize offset=0};
  \draw[refedge] (b1) -- (b2); \draw[refedge] (b2) -- (ai);
  % fork
  \node[font=\small\bfseries] at (2.45,0.65) {fork};
  \node[fsnode] (pfd) at (2.0,0) {Parent fd};
  \node[fsnode] (cfd) at (2.9,0) {Child fd};
  \node[fsnodep] (fofd) at (2.45,-1) {same OFD\\[-1pt]\scriptsize shared offset};
  \node[fsnodeg] (fi) at (2.45,-2) {inode Y};
...
\]
两条路径都采用“逻辑编号 $\to$ 映射结构 $\to$ 物理位置”。增加映射表后，系统可以移动、共享、稀疏分配或扩展底层空间，而不改变上层使用的逻辑编号。
\end{bridge}

\section{第 9 章：inode 多级索引——最大文件与 I/O 次数计算}
...
\begin{center}
\begin{tikzpicture}[x=2.10cm,y=1.20cm]
  \tikzset{
    idxnode/.style={draw=deeporange, thick, fill=softorange, rounded corners=1.5mm, align=center, minimum width=1.85cm, minimum height=0.75cm, font=\scriptsize\bfseries},
    dtanode/.style={draw=deepgreen, thick, fill=softgreen, double, rounded corners=1.5mm, align=center, minimum width=1.85cm, minimum height=0.75cm, font=\scriptsize\bfseries},
    rootnode/.style={draw=mainblue, thick, fill=softblue, rounded corners=1.5mm, align=center, minimum width=1.85cm, minimum height=0.75cm, font=\scriptsize\bfseries},
    levnode/.style={draw=purple, thick, fill=softpurple, rounded corners=1.5mm, align=center, minimum width=1.85cm, minimum height=0.75cm, font=\scriptsize\bfseries}
  }
  \node[rootnode] (ino) at (0,0) {inode\\[-1pt]\tiny mixed index};
  
  \node[levnode] (d) at (1,1.15) {Direct\\[-1pt]\tiny 直接指向};
  \node[levnode] (s) at (1,0.38) {Single\\[-1pt]\tiny 单重间接};
  \node[levnode] (q) at (1,-0.38) {Double\\[-1pt]\tiny 双重间接};
  \node[levnode] (t) at (1,-1.15) {Triple\\[-1pt]\tiny 三重间接};
  
  % Direct
  \node[dtanode] (bd) at (2,1.15) {Data Block\\[-1pt]\tiny 数据块};
  
  % Single
  \node[idxnode] (i1) at (2,0.38) {Index Block\\[-1pt]\tiny 一级索引块};
  \node[dtanode] (bs) at (3,0.38) {Data Block\\[-1pt]\tiny 数据块};
  
  % Double
  \node[idxnode] (i2a) at (2,-0.38) {L2 Index\\[-1pt]\tiny 二级索引表};
  \node[idxnode] (i2b) at (3,-0.38) {L1 Index\\[-1pt]\tiny 一级索引表};
  \node[dtanode] (bq) at (4,-0.38) {Data Block\\[-1pt]\tiny 数据块};
  
  % Triple
  \node[idxnode] (i3a) at (2,-1.15) {L3 Index\\[-1pt]\tiny 三级索引表};
  \node[idxnode] (i3b) at (3,-1.15) {L2 Index\\[-1pt]\tiny 二级索引表};
  \node[idxnode] (i3c) at (4,-1.15) {L1 Index\\[-1pt]\tiny 一级索引表};
  \node[dtanode] (bt) at (5,-1.15) {Data Block\\[-1pt]\tiny 数据块};
...
经典 direct/single/double/triple mixed-indexing 机制非常适合 408 中的索引计算；现代 ext4 可使用 extent tree，把“连续逻辑范围 $\to$ 连续物理范围”作为映射单位。Linux ext4 文档明确区分 inode 的 extent 格式等结构。工程边界用于理解“现代文件系统为何减少大文件映射元数据”，但不替代题设采用的经典 mixed-indexing 机制。
\end{eng}
...
\begin{method}[文件操作状态变化]
\code{create} 建立对象并把名字接入父目录；\code{open} 建立新的 OFD 与 fd；\code{dup}/\code{fork} 可共享同一 OFD 及其 offset；\code{close} 只释放一个句柄引用；\code{read}/\code{write} 推进共享 offset（除非使用显式位置接口）；\code{lseek} 改变 offset 而不读写数据；\code{truncate} 改变文件大小并可能释放数据块；\code{unlink} 删除命名边，仍被打开的对象直到最后一个引用消失才回收。做题时逐项标记 namespace、inode、OFD、fd、offset、data blocks 哪一层改变。
\end{method}
...
\midrule
\endhead
filename & $\neq$ & inode / FCB & 名字属于目录命名关系；inode/FCB 表示持久对象的身份与元数据。 & 把 rename/link/unlink 错写成直接修改对象身份。\\
directory entry & $\neq$ & dentry & 前者可指持久目录记录；Linux dentry 是 VFS 的内存命名缓存对象。 & 把教材磁盘结构与工程内存对象混成一层。\\
fd & $\neq$ & inode & fd 是进程句柄，通常先引用打开文件描述；inode 是持久文件对象。 & 无法解释同一 inode 上多个独立 open 的不同 offset。\\
OFD & $\neq$ & inode & OFD 保存一次打开实例的 offset/flags 等运行时状态；多个 OFD 可以共享同一 inode。 & 把独立 open、dup、fork 的共享关系判错。\\
link count & $\neq$ & open reference & 前者统计持久命名边；后者反映运行时打开引用。 & 误以为 $nlink=0$ 时数据必然立即释放。\\
file offset & $\neq$ & PBN & offset 先定位 file page/LBN，再经文件映射结构得到存储位置。 & 把逻辑文件位置直接当成设备块号。\\
页缓存 & $\neq$ & block mapping & 页缓存按文件对象与页索引保存内容副本；block mapping 在 miss/后端访问时解析存储位置。 & 无条件先求 PBN，错误增加底层 I/O。\\
FS block & $\neq$ & sector / atomic write unit & 文件系统分配单位、设备扇区和设备原子写保证不是同一层概念。 & 错判空间利用率、I/O 粒度或 crash 原子性。\\
\code{write()} 返回 & $\neq$ & durable & 接口返回、后台 writeback、journal commit、\code{fsync()} 和设备稳定介质属于不同时间边界。 & 把“内核已接受”误写成“断电后必然存在”。\\
经典 mixed inode & $\neq$ & ext4 extent & 前者是 408 的经典索引模型；现代 ext4 常使用 extent tree 表示连续范围。 & 用现代实现细节替换题设经典模型，或反过来把经典结构冒充现代事实。\\
\bottomrule
\end{longtable}
\endgroup

\section{第 19 章：最小调用接口}

遇到文件系统问题时，先判当前对象属于 \kw{Name、Object、Open State、Data Mapping 还是 Persistent Protocol}。Canonical 负责解释这些对象怎样存在、引用和变化；固定十四问、路径/打开实例状态追踪与 `unlink` 变式进入 \texttt{路径与文件对象.md}，mixed indexing、字节定位与 I/O 次数进入 \texttt{文件索引与IO计算.md}，crash point、fsck/journaling 与持久边界进入 \texttt{崩溃一致性.md}。

第一观察点始终是：\kw{当前操作改变的是哪一层状态？} 若问题继续进入设备 request、DMA、interrupt 或 block/wakeup，则停止在本册展开并转交 OS-05 / OS-I01。
