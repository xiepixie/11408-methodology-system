...
> 类型：Atlas
> 状态：已采用；README 是 Canonical Subject Atlas。外部操作系统笔记 76 篇全量 Source Diff 已完成，六册 Topic/Foundation Canonical Handbook 均已增量吸收并发布；OS-B01--B04 已完成第二轮语义收敛并重发，OS-I01/OS-I02 已建立并发布 Integration。模拟器只保留生成性算例，UI 描述仍留在 Evidence；新增 Rules 与 Integration 仍待真实题面重复验证和人工确认。
