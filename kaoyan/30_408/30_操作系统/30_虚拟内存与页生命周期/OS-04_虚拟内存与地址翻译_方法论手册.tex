...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \kw{Page Fault（页故障异常）}：页式地址访问无法沿普通快路径完成时由硬件触发的同步异常入口；原因可包括页面不驻留、COW、权限/保护等。408 经典“缺页异常”通常特指页面当前不驻留这一原因。
...
\begin{eng}[系统的通用推理：Page Fault 同步异常中的按序约束]
...
经典静态重定位在 load time 修改需要重定位的位置，使程序以后直接使用本次装入位置对应的地址。优点是执行期硬件简单；缺点是运行后移动困难，灵活性和隔离能力有限。

\begin{warn}[这里的 relocation 与链接器 relocation 不是同一层]
OS-00 中的\kw{link-time relocation}发生在目标文件/可执行映像形成阶段：链接器根据符号解析结果和程序整体布局修补引用；本节的\kw{load-time static relocation}发生在程序实际装入某个内存位置时。两者都可能“改地址相关字段”，但输入信息、执行者和绑定时机不同。做题先问：当前拿到的是 object/executable 文件，还是已经在讨论某次运行的装入位置？
\end{warn}

\subsection{动态重定位：把“地址意义”推迟到运行时}
动态重定位把逻辑/虚拟地址保留下来，直到每次访问时由 MMU 根据当前映射状态翻译。最简单实现是 Base/Limit；分页系统则把映射粒度细化到页。这里的核心不是“程序每次都被重新链接”，而是\kw{运行时地址解释仍由硬件+OS 状态参与}。
...
\noindent\begin{tabularx}{\linewidth}{>{\bfseries}p{0.22\linewidth} X X}
\toprule
...
碎片类型 & 存在页内\kw{内部碎片 (Internal Fragmentation)} & 存在段间\kw{外部碎片 (External Fragmentation)}\\
...
\boxed{
\begin{aligned}
\text{Segment Table Entry}={}&\text{Base}\mid\text{Limit}\mid\text{Present}\\
&\mid\text{Access Rights}\mid\text{Dirty}\mid\text{Backing/Swap Info}
\end{aligned}}
\]
翻译三步走：
\begin{enumerate}
...
\begin{aligned}
\text{Logical Segment}
&\xrightarrow{\text{Segment Table}}\text{Segment Page-Table Base}\\
&\xrightarrow{\text{Page Table}}\text{Physical Frame}
\end{aligned}}
\]
...
\subsection{TLB miss、Page Fault 与 Cache miss 必须彻底分开}
\begingroup
\small
\renewcommand{\arraystretch}{1.25}
\setlength{\tabcolsep}{3pt}
\noindent\begin{tabularx}{\linewidth}{>{\bfseries}p{0.18\linewidth} X X}
\toprule
...
\subsection{有效访问时间 EAT：只提供 VM 路径，不重复拥有性能公式}
408 常把 TLB、页表与缺页慢路径组合后的期望时间记作 EAT（Effective Access Time，有效访问时间）。它的数学底座仍是路径期望：
\[
EAT=\sum_i P(\text{path}_i)\,T(\text{path}_i).
\]
OS-04 只负责给出 normal / fault 路径中的映射、驻留、修复和 retry 事件；hit time、miss total、extra penalty、Cache AMAT 与多级时间口径统一由 CO-06《存储层次与 AMAT》解释。这里做题的关键仍是\kw{明确每条路径到底发生几次 TLB 查询、页表内存访问、最终数据访问，以及 fault 后是否需要 I/O 和 retry}。
...
\subsection{统一处理流程}
\begin{center}
\resizebox{0.96\linewidth}{!}{\input{30_408/30_操作系统/30_虚拟内存与页生命周期/assets/src/page_fault_lifecycle_body.tikz}}
\end{center}
图把 Page Fault 之后的三条真实分支拆开：纯内存修复可直接返回重试，需要外部内容时才阻塞等待 I/O，非法地址或真实权限错误则拒绝访问。PTE 只能在目标页内容已经就绪后再对普通访问开放。
...
\begin{center}
\resizebox{0.96\linewidth}{!}{\input{30_408/30_操作系统/30_虚拟内存与页生命周期/assets/src/clock_replacement_body.tikz}}
\end{center}
Basic CLOCK 的扫描规则可以直接复原；加入 dirty/modification bit 后，图只固定两类状态的语义，不把某一教材的 Enhanced CLOCK 扫描轮次冒充统一标准。
...
\boxed{
\begin{aligned}
\text{fault rate too high}&\Rightarrow\text{try more frames / admission control},\\
...
\section{最小调用接口：先定位地址、映射、驻留还是翻译}

遇到虚拟内存问题时，本册只要求先确定当前问题落在哪一层：\kw{地址表示、页表映射、物理驻留、硬件翻译、fault 修复或回收策略}。若题目开始要求固定计算步骤、页面访问串模拟、题型路由或考场检查顺序，转入同目录训练文档 \texttt{地址翻译与缺页.md} 与 \texttt{页面驻留与置换.md}；若同时追踪 TLB/MMU 与 OS fault handler 的软硬件交接，则转入 X-B02。

第一观察点不是“这像哪种题型”，而是：\kw{当前缺失或变化的究竟是哪一层状态？} 只要这一层没有锁定，就不应继续累加访问次数或套置换算法。
