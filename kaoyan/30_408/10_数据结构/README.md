...
这里要兼容两种常见口径而不制造冲突：408 教材常把“逻辑结构、存储结构、数据运算”并列为数据结构课程的三块核心研究内容；ADT 视角则把抽象状态与操作语义作为公开契约，把具体存储字段和实现算法隐藏在表示内部。前者是在描述课程研究对象，后者是在强调接口与实现分离。因而不能把“数据运算与数据结构无关”写成绝对结论，也不能因为具体算法依赖存储表示，就把操作语义降格成实现细节。

进一步判断名词层级时分三层：\kw{线性表/有序表}首先描述逻辑关系与操作契约；\kw{顺序存储/链式存储/索引/散列}描述关系的编码方法；\kw{顺序表/单链表/双链表}是带具体字段和不变量的表示。数组通常是顺序表示的语言载体，但“数组”本身也可以作为独立数据结构讨论，不能简单等同于“物理结构”这个抽象类别。
...
跨科母模型入口：[缓冲与有限中间态](../00_统一总图/跨科母模型_缓冲与有限中间态.md)。当队列/循环队列或外部排序缓冲开始承担生产者—消费者解耦时，用该母模型理解“为什么要暂存”；队列本体的端点语义与不变量仍由 DS-02/DS-12 拥有。
