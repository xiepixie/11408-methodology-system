...
\begin{center}
\resizebox{0.94\linewidth}{!}{\input{30_408/10_数据结构/03_串与模式匹配/assets/src/kmp_fallback_body.tikz}}
\end{center}
图中只编码“已经匹配部分如何重新对齐”和“主串当前字符仍是同一个”两件事；横向距离只表示对齐关系，不表示真实执行时间。
...
\begin{center}
\resizebox{0.94\linewidth}{!}{\input{30_408/10_数据结构/03_串与模式匹配/assets/src/kmp_nextval_body.tikz}}
\end{center}
图中虚线跳跃只表示“提前删除必败的模式状态”；普通失配仍保持当前主串字符，只修改模式状态。
