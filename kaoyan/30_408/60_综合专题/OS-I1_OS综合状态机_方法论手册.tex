...
\begin{corebox}[核心模型：状态、转换与评价]
进程、虚拟内存、并发、I/O 和文件系统专题分别定义局部机制；这里仅追踪多个机制参与同一过程时的状态变化。
...
Fast / 慢路径 & Cost Optimization & 它只回答“怎样把常见情况做便宜”，不再作为运行定律 \\
...
Evaluation & \kw{哪些状态绝不能出现？怎样更快/更省？} & protection, invariant, liveness, consistency, cache, 快路径 \\
\bottomrule
\end{tabularx}
\end{center}
...
\]
COW 可以延迟复制，但不能破坏进程隔离；writeback 可以延迟持久化，但必须明确 durability 边界；lock 快路径 可以省掉调度，但不能破坏互斥。
\end{warn}

\newpage
\section{Part IV：从模型到题目——五种轨迹模板}

\section{第 10 章：五种常见轨迹模板}

\begin{corebox}[使用方法]
五种轨迹只描述常见时间结构。每次使用仍需填写 $O,R,Q,E,M,\pi,I,C$，不能用轨迹名称替代状态分析。
\end{corebox}
...
\newpage
\section{Part V：九个跨专题过程}

\section{第 12 章：八项分析表}
任何跨专题综合题都填写下面八项：
\begin{enumerate}
  ...
  \item \kw{Cost}：主要成本在哪里？有没有 fast/慢路径？
\end{enumerate}
...
\section{第 13 章：综合题一——程序从 fork 到 exec、运行、exit、wait}
...
Cost & COW 减少 fork 初始复制；fault 把复制成本推迟到写入 慢路径 \\
\bottomrule
\end{tabularx}
\end{center}

\section{第 14 章：综合题二——一次 blocking read(fd,buf,n)}

\subsection{四条信息同时追踪}
综合版不再只画一条调用栈，而同时追踪：\kw{Control、Task State、Data、Reference}。

\begin{center}
\begin{tikzpicture}[x=3.50cm,y=1.70cm]
  \node[knode] (u) at (0,0) {User Task\\[-1pt]\scriptsize Running State};
  \node[knodep] (fs) at (1,0) {File / VFS\\[-1pt]\scriptsize fd $\to$ Open File};
  \node[knodeg] (pc) at (2,0) {页缓存\\[-1pt]\scriptsize Cache Miss};
...
Relations & fd$\to$file$\to$inode；file/page-index$\to$页缓存；request$\to$device \\
...
Cost & page-cache hit 是 快路径；miss 进入 storage 慢路径；copy 与 I/O 是主要成本 \\
\bottomrule
\end{tabularx}
\end{center}

\section{第 15 章：综合题三——file-backed mmap 的首次访问}
...
Relations & VA$\to$VMA；VMA$\to$file+offset；PTE$\to$frame；file page$\to$页缓存 \\
...
Cost & lazy mapping 把实际 I/O 推迟到 first-touch 慢路径；后续访问回到 MMU 快路径 \\
\bottomrule
\end{tabularx}
\end{center}

\section{第 16 章：综合题四——fork + COW + 文件描述符共享}
...
\begin{exam}[跨专题识别点]
题目问“fork 以后是否共享”时禁止回答“共享/不共享”两个字。必须先问：\kw{共享的是哪个对象或哪条关系？} Address space identity、physical frame、fd table、打开文件描述、inode 都不是同一层。
\end{exam}

\section{第 17 章：综合题五——Mutex contention}
...
Cost & uncontended 快路径 vs contended block/schedule 慢路径 \\
\bottomrule
\end{tabularx}
\end{center}

\section{第 18 章：综合题六——Pipe empty/full 与背压}
...
\section{第 19 章：综合题七——Memory Pressure 的一生}
...
这道综合题把 VM、页缓存、File System、I/O 与 Scheduling 接起来：
\begin{itemize}
...
\section{第 20 章：综合题八——write + fsync + crash}
...
Relations & file offset$\to$页缓存；metadata$\to$journal/blocks \\
...
Cost & 延迟写降低前台延迟，却把持久化成本推迟到 writeback/fsync 慢路径 \\
\bottomrule
\end{tabularx}
\end{center}

\section{第 21 章：综合题九——unlink while open}
...
\begin{enumerate}
  ...
  \item 是否发生 fast-path miss，进入 慢路径？
...
\begin{tikzpicture}[x=3.50cm,y=1.70cm]
  ...
  \node[knodeg] (c) at (1,-1) {Cost Check\\[-1pt]\scriptsize Fast / 慢路径};
...
进程、虚拟内存、并发、I/O、文件系统的差异，体现在对象、关系、队列、事件、机制、策略、不变量和成本函数不同；分析步骤保持一致。
\end{corebox}

\section{附录 A：概念归属检查表}
...
Fast/慢路径 & Cost & 比较常见路径与 miss/conflict 的成本差 \\
...
\begin{itemize}
  ...
  \item \kw{I/O 专题}负责 polling、interrupt、DMA、driver、页缓存 与 spooling；综合篇只追踪 request、block、completion 与 wake。
  \item \kw{文件系统专题}负责 path/fd/inode/block/journal；综合篇只关注这些关系怎样跨 VM/I/O/进程状态传播。
\end{itemize}

\begin{method}[使用边界]
专题册负责定义机制；综合册负责让多个机制在同一状态机中协作。因此这里不重复定义页表、PV、inode 或 DMA，只训练跨对象、跨事件和跨不变量的推演。
\end{method}
